...
* Configure on-demand two-way frame delay measurement for an AC.
  ...
  1. Perform the following steps on the devices at both ends of an AC where on-demand two-way frame delay measurement will be implemented
     ...
     4. Perform the following steps on the devices where the MEPs reside:
        ...
        + On the PE, run the [**map**](cmdqueryname=map) **bridge-domain** *bd-id* command to bind the MA to a specified BD.
  ...
  2. Configure a test instance.
     ...
     1. On the receiving device where two-way frame delay measurement will be implemented:
     ...
     2. On the transmitting device where two-way frame delay measurement will be implemented:
  ...
  3. On the receiving device where on-demand two-way frame delay measurement will be implemented, run [**delay-measure two-way receive**](cmdqueryname=delay-measure+two-way+receive) **test-id** *test-id*
...
* Configure proactive two-way frame delay measurement for an AC.
  ...
  2. Configure a test instance.
     ...
     2. Perform the following step on the transmitting device where two-way frame delay measurement will be implemented:
        ...
        Run the [**test-id**](cmdqueryname=test-id) *test-id-value* [ **testid-file** ] **mep** *mep-id* [ **mac** *mac-address* | **remote-mep** *mep-id* ] [ **8021p** *8021p-value* ] [ **description** *description* ] command to configure a test instance.
  3. On the receiving device where proactive two-way frame delay measurement will be implemented, run [**delay-measure two-way receive**](cmdqueryname=delay-measure+two-way+receive) **test-id** *test-id*
