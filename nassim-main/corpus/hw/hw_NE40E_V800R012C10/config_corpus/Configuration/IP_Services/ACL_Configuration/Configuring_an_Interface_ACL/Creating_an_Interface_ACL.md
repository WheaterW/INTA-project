...
2. Run [**acl**](cmdqueryname=acl+name+interface+interface+number+number+match-order+config) { **name** *interface-based-acl-name* { **interface** | [ **interface** ] **number** *interface-based-acl-number* } | [ **number** ] *interface-based-acl-number* } [ **match-order** { **config** | **auto** } ]
