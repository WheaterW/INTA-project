...
This section describes how to configure MVPNv4 over BIERv6 on a unicast L3VPNv4 over SRv6 network.

#### Networking Requirements

In [Figure 1](#EN-US_TASK_0251394660__fig35284409229), L3VPNv4 over SRv6 has been deployed. It is required that MVPNv4 over BIERv6 be deployed on the existing network.

**Figure 1** MVPNv4 over BIERv6 networking![](../../../../public_sys-resources/note_3.0-en-us.png) 

In this example, interface 1, interface 2, and interface 3 represent GE 0/1/0, GE 0/1/1, and GE 0/1/2, respectively.


  
![](figure/en-us_image_0283195892.png "Click to enlarge")
...
* IP addresses of main interfaces on each device, as shown in [Figure 1](#EN-US_TASK_0251394660__fig35284409229)
...
1. (Optional) Configure L3VPNv4 over SRv6.
   ...
   # Enable IPv6 forwarding on each interface and configure an IPv6 address for each interface. The configuration of PE1 is used as an example. The configurations of PE2, PE3, and the P are similar to the configuration of PE1. For detailed configurations, see Configuration Files.
   ...
   # Configure IS-IS. The configuration of PE1 is used as an example. The configurations of PE2, PE3, and the P are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0251394660__postreq24192593172748) in this section.
   ...
   # Configure a VPN instance on each PE, enable the IPv4 address family for the instance, and bind the interface that connects each PE to a CE to the VPN instance on that PE. The configuration of PE1 is used as an example. The configurations of PE2 and PE3 are similar to the configuration of PE1.
   ...
   # Establish an MP-IBGP peer relationship between PE1 and PE2 and between PE1 and PE3. The configuration of PE1 is used as an example. The configurations of PE2 and PE3 are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0251394660__postreq24192593172748) in this section. After you run the [**peer enable**](cmdqueryname=peer+enable) command, enter **Y** at the system prompt to confirm the action.
...
3. Establish BGP MVPN peer relationships between PEs.
   ...
   # Display information about BGP MVPN peers. The command output shows that a BGP MVPN peer relationship has been established between PE1 and PE2 and between PE1 and PE3.
