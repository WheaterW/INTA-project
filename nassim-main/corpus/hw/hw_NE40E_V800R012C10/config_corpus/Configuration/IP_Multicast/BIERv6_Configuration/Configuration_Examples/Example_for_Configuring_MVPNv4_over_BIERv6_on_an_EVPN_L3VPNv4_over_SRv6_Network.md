...
This section describes how to configure MVPNv4 over BIERv6 on a unicast EVPN L3VPNv4 over SRv6 network.

#### Networking Requirements

In [Figure 1](#EN-US_TASK_0251394661__fig35284409229), EVPN L3VPNv4 over SRv6 has been deployed. It is required that MVPNv4 over BIERv6 be deployed on the existing network.
...
![](figure/en-us_image_0283435060.png "Click to enlarge")
#### Configuration Roadmap

The configuration roadmap is as follows:

1. (Optional) Configure EVPN L3VPNv4 over SRv6 and ensure that the unicast VPN runs properly. If the unicast network has been configured, skip this step.
...
* IP addresses of key interfaces on each device, as shown in [Figure 1](#EN-US_TASK_0251394661__fig35284409229)
...
1. (Optional) Configure EVPN L3VPNv4 over SRv6.
   ...
   # Enable IPv6 forwarding on each interface and configure an IPv6 address for each interface. The configuration of PE1 is used as an example. The configurations of PE2, PE3, and the P are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0251394661__postreq24192593172748) in this section. Similar details will be omitted in the rest of the document.
   ...
   ```
   ```
   [*PE1-vpn-instance-VPNA-af-ipv4-mvpn] vpn-target 111:1 both
   ```
   ```
   ...
   ```
   ```
   [*PE1-bgp-VPNA] segment-routing ipv6 best-effort evpn
   ```
   ```
...
4. Configure multicast traffic forwarding over a BIERv6 I-PMSI tunnel.
   ...
   # Display I-PMSI tunnel information of a specified VPN instance. The command output shows that an I-PMSI tunnel has been established, with PE1 as the root node and PE2 and PE3 as leaf nodes.
