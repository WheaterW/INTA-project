...
* [Configure EBGP between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002501)
* [Configure IBGP between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002502)
* [Configure a static route between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002503)
* [Configure RIP between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002504)
* [Configure OSPF between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002505)
* [Configure IS-IS between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002506)
* [Configure a direct route between a PE and a CE.](#EN-US_TASK_0000001270193545__step_dc_vrp_cfg_ngmvpn_002507)
...
* Configure EBGP between a PE and a CE.
  ...
  4. (Optional) Run [**as-number**](cmdqueryname=as-number) *vrf-as* [ *no-prepend* ]
     
     
     
     An AS number is configured for the VPN instance IPv4 address family.
     
     If a device needs to be simulated as multiple BGP devices logically during network transfer or service identification, you can run the [**as-number**](cmdqueryname=as-number) command to configure a different AS number for each VPN instance IPv4 address family.
  ...
  8. (Optional) Run [**peer**](cmdqueryname=peer) { *group-name* | *ipv4-address* } [**soo**](cmdqueryname=soo) *site-of-origin*
  ...
  10. (Optional) Run [**peer**](cmdqueryname=peer) *ipv4-address* [**substitute-as**](cmdqueryname=substitute-as)
  ...
  4. (Optional) Run [**peer**](cmdqueryname=peer) { *ipv4-address* | *group-name* } [**ebgp-max-hop**](cmdqueryname=ebgp-max-hop) [ *hop-count* ]
  ...
  5. Run [**import-route**](cmdqueryname=import-route) { **direct** | **static** | **rip** *process-id* | **ospf** *process-id* | **isis** *process-id* } [ **med** *med* | **route-policy** *route-policy-name* ] \*
...
* Configure IBGP between a PE and a CE.
  ...
  3. Run [**peer**](cmdqueryname=peer) *ipv4-address* [**as-number**](cmdqueryname=as-number) *as-number*
     ...
     The PE is configured as a VPN peer.
  4. Run [**import-route**](cmdqueryname=import-route) { **direct** | **static** | **rip** *process-id* | **ospf** *process-id* | **isis** *process-id* } [ **med** *med* | **route-policy** *route-policy-name* ] \*
...
* Configure RIP between a PE and a CE.
  ...
  Configuring RIPv1 or RIPv2 on the CE is not described here. For details about how to configure RIP, see "RIP Configuration" in the *[HUAWEI NE40E-M2 seriesUniversal Service Router Configuration Guide - IP Routing](dc_vrp_rip_cfg_0000.html)*.
  ...
  8. Run [**import-route**](cmdqueryname=import-route) **rip** *process-id* [ **med** *med* | **route-policy** *route-policy-name* ] \*
     ...
     After the [**import-route**](cmdqueryname=import-route) **rip** command is run in the BGP-VPN instance IPv4 address family view, the PE imports the VPN routes learned from the attached CE into the BGP routing table and advertises VPNv4 routes to the remote PE.
...
* Configure OSPF between a PE and a CE.
  ...
  Configuring OSPF on the CE is not described here. For details about how to configure OSPF, see "OSPF Configuration" in the *[HUAWEI NE40E-M2 seriesUniversal Service Router Configuration Guide - IP Routing](dc_vrp_ospf_cfg_0000.html)*.
...
* Configure IS-IS between a PE and a CE.
  ...
  Configuring IS-IS on the CE is not described here. For details about how to configure IS-IS, see "IS-IS Configuration" in the *[HUAWEI NE40E-M2 seriesUniversal Service Router Configuration Guide - IP Routing](dc_vrp_isis_cfg_0000.html)*.
...
* Configure a direct route between a PE and a CE.
  ...
  4. Run [**arp vlink-direct-route advertise**](cmdqueryname=arp+vlink-direct-route+advertise) [ **route-policy** *route-policy-name* | **route-filter** *route-filter-name* ]
     ...
     After the parameter **route-policy** *route-policy-name* or **route-filter** *route-filter-name* is specified in the [**arp vlink-direct-route advertise**](cmdqueryname=arp+vlink-direct-route+advertise) command, only filtered ARP Vlink direct routes are advertised.
  5. Run [**quit**](cmdqueryname=quit)
