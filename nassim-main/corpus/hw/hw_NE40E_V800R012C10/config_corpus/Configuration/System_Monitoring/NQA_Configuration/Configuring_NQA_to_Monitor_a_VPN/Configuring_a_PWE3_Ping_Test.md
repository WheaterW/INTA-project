...
2. Create an NQA test instance and set its type to PWE3 ping.
   ...
   2. Run the [**test-type**](cmdqueryname=test-type) **pwe3ping** command to set the test instance type to PWE3 ping.
...
4. Set parameters for the L2VPN network to be tested.
   ...
   2. Run the [**local-pw-type**](cmdqueryname=local-pw-type) *pwTypeValue* command to configure an encapsulation type for the local PW.
   ...
   4. Run the [**local-pw-id**](cmdqueryname=local-pw-id) *pwIdValue* command to configure a local PW ID.
   5. (Optional) Run the [**peer-address**](cmdqueryname=peer-address) *peeraddress* command to configure the peer IP address.
   6. (Optional) Run the [**ttl-copymode**](cmdqueryname=ttl-copymode) { **pipe** | **uniform** } command to specify a TTL propagation mode.
5. (Optional) Configure information about the remote PE when a multi-segment PW (MS-PW) is to be tested.
   ...
   1. Run the [**remote-pw-id**](cmdqueryname=remote-pw-id) *pwIdValue* command to configure a PW ID for the remote PE.
   2. Run the [**destination-address**](cmdqueryname=destination-address) **ipv4** *destAddress* command to configure an IP address for the remote PE.
   3. (Optional) Run the [**sender-address**](cmdqueryname=sender-address) **ipv4** *ip-address* command to configure a source IP address for the public network session between the device and the remote PE. This IP address is usually the IP address of a superstratum provider edge (SPE) that switches and forwards adjacent labels or a user-end provider edge (UPE), which is an edge device on a backbone network.
