...
On the inter-AS BGP VPLS Option B network shown in [Figure 1](#EN-US_TASK_0190963351__fig_dc_vrp_vpls_cfg_602101), ASBRs advertise routes and function as PEs to manage VPN routes. This reduces the number of PEs to be deployed and increases the performance requirements for ASBRs.

**Figure 1** Configuring inter-AS BGP VPLS in Option B mode (ASBRs function as PEs)![](../../../../public_sys-resources/note_3.0-en-us.png) 
...
![](figure/en-us_image_0191796483.png)

#### Configuration Roadmap

The configuration roadmap is as follows:

1. Establish an EBGP peer relationship between ASBRs, enable the L2VPN-AD address family, and trigger the establishment of a local IFNET tunnel. Configure a common BGP VPLS service and bind the corresponding AC interface to the service.
...
1. Configure interface IP addresses.
   ...
   ```
   ```
   [*ASBR2-GigabitEthernet0/2/0] ip address 10.10.2.3 24
   ```
   ```
...
5. Configure MP-IBGP peer relationships in each AS.
   ...
   ```
   ```
   [*ASBR2-bgp] peer 10.10.2.2 connect-interface gigabitethernet 0/2/0
   ```
   ```
...
7. Verify the configuration.
   ...
   * ASBR2 configuration file
     ...
     bgp 200
      ...
      peer 10.10.2.2 connect-interface GigabitEthernet0/2/0
      #
      l2vpn-ad-family
       undo policy vpn-target
       tunnel-selector s1  
