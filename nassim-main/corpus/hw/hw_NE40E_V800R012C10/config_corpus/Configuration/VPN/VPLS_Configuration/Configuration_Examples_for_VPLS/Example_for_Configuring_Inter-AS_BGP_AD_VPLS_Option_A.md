...
Inter-AS BGP AD VPLS Option A is recommended for scenarios where few inter-AS VPLS PWs are required.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0172370242__fig_dc_vrp_vpls_cfg_605201), CE1 and CE2 access the MPLS backbone network through PE1 in AS 100 and PE2 in AS 200, respectively.

Inter-AS BGP AD VPLS Option A needs to be deployed for CE1 and CE2 to communicate.

**Figure 1** Configuring inter-AS BGP AD VPLS Option A![](../../../../public_sys-resources/note_3.0-en-us.png) 
...
![](images/fig_dc_vrp_vpls_cfg_605201.png)
...
3. Configure basic MPLS functions and establish LSPs.
   ...
   [*PE2] commit
   ```
4. Establish an MP-IBGP peer relationship between the ASBR and PE in the same AS.
   ...
   After completing the configurations, run the [**display bgp l2vpn-ad peer**](cmdqueryname=display+bgp+l2vpn-ad+peer) command on each PE or ASBR. The command output shows that the MP-IBGP peer relationship status is **Established**. The following example uses the command output on PE1.
...
5. Configure BGP AD VPLS.
   ...
   ```
   ```
   [*ASBR1-GigabitEthernet0/2/0.1] l2 binding vsi vplsad1
   ```
   ```
   ...
   ```
   ```
   [*ASBR2-GigabitEthernet0/1/0.1] l2 binding vsi vplsad1
   ```
   ```
...
6. Configure CEs to allow packets from VLAN 10 to pass through.
   ...
   After completing the configurations, run the [**display vsi**](cmdqueryname=display+vsi) command on each PE or ASBR. The command output shows that the VSI status is **up**. The following example uses the command output on PE1.
