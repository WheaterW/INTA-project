Example 1 for Configuring Hub-Spoke (Single Link Between a Hub-PE and a Hub-CE)
===============================================================================

On a VPN with Hub-Spoke networking, all users communicate with each other through a specified access control device.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0172369472__fig_dc_vrp_mpls-l3vpn-v4_cfg_022002), the communication between Spoke-CEs is controlled by the Hub-CE at a central site. In other words, the traffic between Spoke-CEs is forwarded through the Hub-CE, not only through the Hub-PE. The Hub-PE connects to the Hub-CE over a link.

**Figure 1** Example 1 for configuring Hub-Spoke (single link between the Hub-PE and Hub-CE)![](../../../../public_sys-resources/note_3.0-en-us.png) 

Interfaces 1 through 3 in this example represent GE 0/1/0, GE 0/2/0, and GE 0/3/0, respectively.


  
![](images/fig_dc_vrp_mpls-l3vpn-v4_cfg_010902.png)
...
1. Establish MP-IBGP peer relationships between the Hub-PE and Spoke-PEs. There is no need to establish an MP-IBGP peer relationship or exchange VPN routing information between the two Spoke-PEs.
2. Create a VPN instance on each PE and enable the pop-go function on the Hub-PE.
3. Configure EBGP connections between CEs and PEs. Configure the Hub-CE to advertise the default route to the Hub-PE.
...
* MPLS LSR IDs of PEs
* Names, RDs, and VPN targets of the VPN instances on the Hub-PE and Spoke-PEs
...
1. Configure IGP on the backbone network for the Hub-PE and Spoke-PEs to communicate.
   ...
   After the configurations are complete, OSPF neighbor relationships are established between the Hub-PE and Spoke-PEs. Run the **display ospf peer** command. The command output shows that the neighbor status is **Full**. Run the **display ip routing-table** command. The command output shows that the Hub-PE and Spoke-PEs have learned the routes to each other's loopback interface.
2. Configure basic MPLS capabilities and enable MPLS LDP to establish LDP LSPs on the backbone network.
...
3. Configure a VPN instance enabled with the IPv4 address family on each PE and bind the interface that connects a PE to a CE to the VPN instance on that PE.
   ...
   # Configure IP addresses for interfaces on CEs, as shown in [Figure 1](#EN-US_TASK_0172369472__fig_dc_vrp_mpls-l3vpn-v4_cfg_022002). For configuration details, see the configuration files.
   
   After completing the configurations, run the **display ip vpn-instance verbose** command on PEs to check the configurations of VPN instances. Each PE can ping its connected CEs through the **ping -vpn-instance** *vpn-name ip-address* command.
   
   ![](../../../../public_sys-resources/note_3.0-en-us.png) 
   
   If a PE has multiple interfaces bound to the same VPN instance, use the **-a** *source-ip-address* parameter to specify a source IP address when running the **ping -vpn-instance** *vpn-instance-name* **-a** *source-ip-address dest-ip-address* command to ping the CE connected to the remote PE. If the source IP address is not specified, the ping operation may fail.
4. Establish EBGP peer relationships between PEs and CEs to import VPN routes.
