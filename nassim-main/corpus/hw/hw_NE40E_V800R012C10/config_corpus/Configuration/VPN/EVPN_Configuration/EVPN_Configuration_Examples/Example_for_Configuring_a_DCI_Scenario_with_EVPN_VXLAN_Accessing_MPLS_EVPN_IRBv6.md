Example for Configuring a DCI Scenario with EVPN VXLAN Accessing MPLS EVPN IRBv6
================================================================================

This section provides an example for configuring a DCI scenario with EVPN VXLAN accessing MPLS EVPN IRBv6. In this example, DC GWs connect to the DCI backbone network over VXLAN tunnels and BGP EVPN is deployed on the DCI backbone network to achieve DC interworking.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0172370691__fig_dc_vrp_dci_cfg_0030_ipv601), the DC GWs GW1 and GW2 are connected to the DCI backbone network with BGP EVPN configured. After EVPN and VXLAN tunnels are deployed between the DC GWs and DCI-PEs, VM host IPv6 routes can be exchanged between different DCs, implementing communication between DC A and DC B (for example, communication between VMa1 and VMb2).

**Figure 1** Configuring a DCI scenario with EVPN VXLAN accessing MPLS EVPN IRBv6![](../../../../public_sys-resources/note_3.0-en-us.png) 
...
1. Configure interface IP addresses, including loopback interface addresses, on each node.
   ...
   For details about how to configure interface IP addresses and masks, see [Configuration Files](#EN-US_TASK_0172370691__dc_vrp_dci_cfg_0030_ipv601).
2. Configure an IGP on the DCI backbone network. OSPF is used in this example.
   
   
   
   For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370691__dc_vrp_dci_cfg_0030_ipv601).
...
* DCI-PE1 configuration file
  ...
  bgp 100
   ...
   l2vpn-family evpn
    ...
    peer 3.3.3.3 advertise route-reoriginated evpn mac-ipv6
    ...
    peer 4.4.4.4 advertise route-reoriginated evpn mac-ipv6
    peer 4.4.4.4 advertise route-reoriginated evpn ipv6
  #               
  ospf 1
...
* DCI-PE2 configuration file
  ...
  bgp 100
   ...
   l2vpn-family evpn
    ...
    peer 1.1.1.1 advertise route-reoriginated evpn mac-ipv6
    peer 1.1.1.1 advertise route-reoriginated evpn ipv6
    peer 5.5.5.5 enable
    peer 5.5.5.5 advertise irbv6
    peer 5.5.5.5 advertise encap-type vxlan
    peer 5.5.5.5 import reoriginate
    peer 5.5.5.5 advertise route-reoriginated evpn mac-ipv6
