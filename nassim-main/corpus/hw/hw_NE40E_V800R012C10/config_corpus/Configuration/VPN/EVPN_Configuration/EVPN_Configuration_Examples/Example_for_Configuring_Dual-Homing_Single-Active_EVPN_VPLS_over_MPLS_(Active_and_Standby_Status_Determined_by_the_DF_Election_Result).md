Example for Configuring Dual-Homing Single-Active EVPN VPLS over MPLS (Active/Standby Status Determined by the DF Election Result)
==================================================================================================================================
...
On the network shown in [Figure 1](#EN-US_TASK_0280709493__fig_dc_vrp_dci_cfg_002601), to allow sites to communicate over the backbone network, configure EVPN so that the sites can exchange EVPN routes to transmit Layer 2 traffic. In a scenario where a CE is dual-homed to PEs in single-active mode, you can configure the PEs to determine the active/standby status based on the DF election result. In addition, local-remote FRR and BFD need to be configured to improve reliability and traffic switching performance.

**Figure 1** Configuring dual-homing single-active EVPN VPLS over MPLS (active/standby status determined by the DF election result)![](../../../../public_sys-resources/note_3.0-en-us.png) 
...
![](figure/en-us_image_0280709494.png "Click to enlarge")
...
6. Configure BFD sessions to monitor access-side interfaces on PE1 and PE2.
7. Establish BGP EVPN peer relationships.
8. Configure CEs to access PEs through physical interfaces.
9. Associate CFM with EVPN DF election.
...
1. Configure interface addresses on the RR and PEs according to [Figure 1](#EN-US_TASK_0280709493__fig_dc_vrp_dci_cfg_002601). For detailed configurations, see Configuration Files.
...
6. Configure an ESI and the dual-homing single-active mode; configure the PEs to determine their active/standby status based on the DF election result.
   ...
   ```
   ```
   [*PE1-GigabitEthernet0/1/0.1] encapsulation dot1q vid 2
   ```
   ```
   ...
   ```
   ```
   [*PE2-GigabitEthernet0/1/0.1] encapsulation dot1q vid 2
   ```
   ```
   ...
   ```
   ```
   [*PE3-GigabitEthernet0/2/0.1] encapsulation dot1q vid 2
   ```
   ```
   [*PE3-GigabitEthernet0/2/0.1] rewrite pop single
   ```
   ```
   [*PE3-GigabitEthernet0/2/0.1] bridge-domain 10
   ```
   ```
...
7. Configure local-remote FRR for MAC routes.
   ...
   [*PE2] commit
   ```
8. Configure BFD sessions to monitor access-side interfaces on PE1 and PE2.
   ...
   ```
   ```
   [*PE1] bfd bfd1 bind peer-ip 2.2.2.2 track-interface interface GigabitEthernet0/1/0
   ```
   ```
   [*PE1-bfd-session-bfd1] discriminator local 1
   [*PE1-bfd-session-bfd1] discriminator remote 2
   [*PE1-bfd-session-bfd1] quit
   ```
   ```
   [*PE1] bfd bfd2 bind peer-ip 2.2.2.2 source-ip 1.1.1.1
   [*PE1-bfd-session-bfd2] discriminator local 3
   [*PE1-bfd-session-bfd2] discriminator remote 4
   [*PE1-bfd-session-bfd2] quit
   ```
   ```
   [*PE1] interface gigabitethernet 0/1/0
   [*PE1-GigabitEthernet0/1/0] es track bfd bfd2
   [*PE1-GigabitEthernet0/1/0] quit
   ```
   ```
   [*PE1] commit
   ...
   ```
   ```
   [*PE2] bfd bfd1 bind peer-ip 1.1.1.1 source-ip 2.2.2.2
   ```
   ```
   [*PE2-bfd-session-bfd1] discriminator local 2
   ```
   ```
   [*PE2-bfd-session-bfd1] discriminator remote 1
   ```
   ```
   [*PE2-bfd-session-bfd1] quit
   ```
   ```
   [*PE2] bfd bfd2 bind peer-ip 1.1.1.1 track-interface interface GigabitEthernet0/1/0
   ```
   ```
   [*PE2-bfd-session-pe2tope1] discriminator local 4
   [*PE2-bfd-session-pe2tope1] discriminator remote 3
   [*PE2-bfd-session-pe2tope1] quit
   ```
   ```
   [*PE2] interface gigabitethernet 0/1/0
   [*PE2-GigabitEthernet0/1/0] es track bfd bfd1
   [*PE2-GigabitEthernet0/1/0] quit
   ```
   ```
   [*PE2] commit
   ```
9. Establish BGP EVPN peer relationships.
   ...
   ```
10. Configure CEs to access PEs through physical interfaces.
    ...
    [*CE1-GigabitEthernet0/1/0] port trunk allow-pass vlan 2
    ```
    ```
    ...
    [*CE1-GigabitEthernet0/2/0] port trunk allow-pass vlan 2
    ```
    ```
    ...
    [*CE2-GigabitEthernet0/1/0] port trunk allow-pass vlan 2
    ```
    ```
    ...
    [*CE2] commit
    ```
11. Associate CFM with EVPN DF election.
    ...
    [*CE1] cfm md 1001
    [*CE1-md-1001] ma 4000
    [*CE1-md-1001-ma-4000] map vlan 2
    ...
    [*CE1-md-1001] ma 6000
    [*CE1-md-1001-ma-6000] map vlan 2
    ...
    [*CE1-md-1001] quit
    [*CE1] cfm trigger vlan 2 mac-renew
    [*CE1] commit
    ```
    ...
    [*PE1] cfm md 1001
    [*PE1-md-1001] ma 4000
    [*PE1-md-1001-ma-4000] map bridge-domain 10
    [*PE1-md-1001-ma-4000] ccm-interval 10
    [*PE1-md-1001-ma-4000] mep mep-id 4000 interface GigabitEthernet0/1/0.1 vlan 2 outward
    ...
    [*PE2] cfm md 1001
    [*PE2-md-1001] ma 6000
    [*PE2-md-1001-ma-6000] map bridge-domain 10
    [*PE2-md-1001-ma-6000] ccm-interval 10
    [*PE2-md-1001-ma-6000] mep mep-id 8000 interface GigabitEthernet0/1/0.1 vlan 2 outward
    ...
    [*PE2] commit
    ```
12. Configure IP addresses on the same network segment on CE1 and CE2 and perform a ping operation on the same network segment.
...
13. Verify the configuration.
    ...
    ESI: 0000.1111.2222.1111.1111
     ...
     IFName GigabitEthernet 0/1/0.1:
      DF Result    : Primary
    ```
    ```
    [~PE2] display evpn vpn-instance name evpna df result
    ESI Count: 1
    ...
    # Run the **display bgp evpn all routing-table mac-route** command on PE3. The command output shows the MAC route destined for CE1 sent from the remote PE. According to the DF election result, PE1 is in the active state, so that PE1 on the primary path functions as the next hop in the MAC route destined for CE1.
...
* PE1 configuration file
  ...
  #
  bfd bfd1 bind peer-ip 2.2.2.2 track-interface interface GigabitEthernet0/1/0                           
   discriminator local 1                                                          
   discriminator remote 2 
  #
  bfd bfd2 bind peer-ip 2.2.2.2 source-ip 1.1.1.1
  ...
  cfm md 1001
   ma 4000
    ...
    ccm-interval 10
    mep mep-id 4000 interface GigabitEthernet0/1/0.1 vlan 2 outward
...
* PE2 configuration file
  ...
  #
  bfd bfd1 bind peer-ip 1.1.1.1 source-ip 2.2.2.2                           
   discriminator local 2                                                          
   discriminator remote 1 
  #
  bfd bfd2 bind peer-ip 1.1.1.1 track-interface interface GigabitEthernet0/1/0
  ...
  cfm md 1001
   ma 6000
    map bridge-domain 10
    ccm-interval 10
    mep mep-id 8000 interface GigabitEthernet0/1/0.1 vlan 2 outward
