...
| --- | --- | --- | --- | --- |
| MPLS TE tunnel (without Ps on links at the network layer) | Protects access rings, aggregation rings, and links at the core layer. | All nodes | [Configure BFD for interface.](#EN-US_TASK_0172368644__BFD_interface) | Configure either of the following TE FRR functions:   * [Configure TE manual FRR.](#EN-US_TASK_0172368644__TE_FRR1) * [Configure MPLS TE Auto FRR.](#EN-US_TASK_0172368644__TE_FRR2) |
| MPLS TE tunnel (with Ps on links at the network layer) | Protects access rings, aggregation rings, and links at the core layer. | All nodes | [Configure static BFD for CR-LSP](#EN-US_TASK_0172368644__BFD_CR-LSP1) or [dynamic BFD for CR-LSP.](#EN-US_TASK_0172368644__BFD_CR-LSP2) | [Configure CR-LSP hot standby.](#EN-US_TASK_0172368644__Hotstandby2) |
| MPLS TE tunnel | Protects AGGs and core ABRs. | All nodes | [Configure BFD for TE.](#EN-US_TASK_0172368644__BFD_TE) | Configure BGP LSP FRR. |
| MPLS LDP LSP | Protects access and aggregation rings, as well as links, AGGs, and core ABRs at the core layer. | All nodes | [Configure static BFD to monitor an LDP LSP](#EN-US_TASK_0172368644__BFD_LDP1) or [dynamic BFD for LDP LSPs.](#EN-US_TASK_0172368644__BFD_LDP2) | Configure BGP LSP FRR. |
| MPLS TE tunnel or MPLS LDP LSP | Protects a whole BGP LSP and MP-BGP peers on an L3VPN. | CSGs and MASGs | [Configure BFD for BGP tunnel.](#EN-US_TASK_0172368644__BFD_BGPLSP) | Configure either of the following VPN FRR functions:   * [Enable VPN FRR in the VPN instance IPv4 address family view.](#EN-US_TASK_0172368644__VPN1) * [Enable VPN FRR in the BGP-VPN instance IPv4 address family view.](#EN-US_TASK_0172368644__VPN2) |
...
* Configure BFD for TE.
  ...
  5. Run [**bfd session**](cmdqueryname=bfd+session) **session-name**
     
     
     
     A session is added to the BFD session group and the BFD session view is displayed.
  6. Run [**discriminator**](cmdqueryname=discriminator+local) **local** *discr-value*
     
     
     
     The local discriminator of the BFD session is configured.
  7. Run [**discriminator**](cmdqueryname=discriminator+remote) **remote** *discr-value*
     ...
     The local discriminator of the local device and the remote discriminator of the remote device are the same, and the remote discriminator of the local device and the local discriminator of the remote device are the same. A discriminator inconsistency causes the BFD session to fail to be established.
  8. Run [**process-pst**](cmdqueryname=process-pst)
     ...
     If the BFD session on a trunk or VLAN member interface allows BFD to modify the port status table or link status table, and the interface is configured with the BFD session, you must configure the WTR time for the BFD session for detecting the interface. This prevents the BFD session on the interface from flapping when the member interface joins or leave the interface.
  9. (Optional) Run [**min-tx-interval**](cmdqueryname=min-tx-interval) *tx-interval*
     
     
     
     The minimum interval at which BFD packets are sent is configured.
  10. (Optional) Run [**min-rx-interval**](cmdqueryname=min-rx-interval) *rx-interval*
      
      
      
      The local minimum interval at which BFD packets are received is configured.
  11. (Optional) Run [**detect-multiplier**](cmdqueryname=detect-multiplier) *multiplier*
...
* Configure BGP LSP FRR.
  ...
  7. (Optional) Run [**route-select delay**](cmdqueryname=route-select+delay) *delay-value*
     ...
     A delay for selecting a route is configured. After the primary path recovers, the device on the primary path performs route selection only after the corresponding forwarding entries on the device are stable. This prevents traffic loss during traffic switchback.
...
* Configure BFD for BGP tunnel.
  ...
  6. Run [**mpls bgp bfd-trigger-tunnel**](cmdqueryname=mpls+bgp+bfd-trigger-tunnel+host+ip-prefix) { **host** | **ip-prefix** *ip-prefix-name* }
     ...
     A policy for dynamically establishing BGP BFD sessions is configured.
  7. Run [**commit**](cmdqueryname=commit)
...
* Enable VPN FRR in the VPN instance IPv4 address family view.
  ...
  7. (Optional) Run [**bgp**](cmdqueryname=bgp) *as-number*
     ...
     The BGP view is displayed.
  8. (Optional) Run [**ipv4-family**](cmdqueryname=ipv4-family+vpn-instance) **vpn-instance** *vpn-instance-name*
