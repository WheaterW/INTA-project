Example for Configuring EVPN VPWS over OSPFv3 SRv6 TE Policy (Manual Configuration)
===================================================================================
...
On the network shown in [Figure 1](#EN-US_TASK_0000001216676927__fig1367823242219), PE1, the P, and PE2 are in the same AS and need to run OSPFv3 to implement IPv6 network connectivity. In addition, a bidirectional SRv6 TE Policy needs to be deployed between PE1 and PE2 to carry EVPN VPWSs.
...
![](figure/en-us_image_0000001171118556.png)
...
5. Configure SRv6 SIDs and enable OSPFv3 SRv6 on PE1, the P, and PE2. In addition, configure PE1 and PE2 to advertise VPN routes carrying SIDs.
6. Deploy an SRv6 TE Policy between PE1 and PE2.
...
1. Enable IPv6 forwarding and configure an IPv6 address for each interface.
   ...
   An IPv4 address needs to be configured for the loopback interface, because the EVPN source address needs to be an IPv4 address.
2. Configure OSPFv3.
   ...
   # Display OSPFv3 neighbor information. The following uses PE1 as an example.
   ...
   ```
3. Configure EVPN and EVPL instances on each PE, bind the EVPL instance to an access-side sub-interface, and configure a VLAN on each CE.
...
4. Establish a BGP EVPN peer relationship between the PEs.
   ...
   ```
5. Configure SRv6 SIDs and configure the PEs to advertise VPN routes carrying SIDs.
...
8. Verify the configuration.
   ...
   Run the **display bgp evpn all routing-table** command on each PE. The command output shows EVPN route information sent from the peer end.
