Example for Configuring Inter-AS EVPN L3VPNv4 over SRv6 TE Policy to Evolve from Non-Compression to Compression
===============================================================================================================

This section provides an example for configuring inter-AS EVPN L3VPNv4 over SRv6 TE Policy to evolve from non-compression to compression.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0000001559771904__fig_dc_vrp_srv6_cfg_all_001101):
...
A bidirectional inter-AS SRv6 TE Policy needs to be deployed between PE1 and PE2 to carry EVPN L3VPNv4 services.
...
![](figure/en-us_image_0000001559931892.png "Click to enlarge")

#### Configuration Roadmap

The configuration roadmap is as follows:

1. Enable IPv6 forwarding and configure IPv6 addresses for interfaces on PE1, PE2, ASBR1, and ASBR2.
2. Enable IS-IS, configure an IS-IS level, and specify a network entity title on PE1, PE2, ASBR1, and ASBR2.
3. Configure an EVPN L3VPN instance on PE1 and PE2 and bind the instance to an access-side interface.
4. Establish an EBGP peer relationship between each PE and its connected CE.
5. Create locator as1 on the PEs, configure intra-AS static End.X SIDs, and configure IS-IS to advertise the SRv6 locator routes of as1.
6. Configure BGP EPEv6 on ASBR1 and ASBR2 to reference locator as1, and allocate inter-AS static End.X SIDs.
...
10. Create locator as2 on the PEs, configure intra-AS static End.X SIDs, and configure IS-IS to advertise the SRv6 locator routes of as2.
11. Configure BGP EPEv6 on ASBR1 and ASBR2 to reference locator as2, and allocate inter-AS static End.X SIDs.
12. Deploy an SRv6 TE Policy between PE1 and PE2 based on locator as2 and add a segment list.
13. Switch to the compressed locator as2 in the BGP VPN instance view of PE1 and PE2.

#### Precautions

During the configuration process, note the following:

* When configuring BGP EPEv6, you need to enable the BGP-LS address family peer relationship for BGP EPEv6 to take effect.

#### Data Preparation

To complete the configuration, you need the following data:

* IPv6 address of each involved device interface
...
1. Enable IPv6 forwarding and configure an IPv6 address for each involved interface.
   ...
   # Configure PE1. The configurations of PE2, ASBR1, and ASBR2 are similar to the configuration of PE1. For configuration details, see the configuration files.
...
3. Create a VPN instance and enable the IPv4 address family on each PE. Then, bind each PE's interface connected to a CE to the corresponding VPN instance.
   ...
   # Configure an IP address for each interface on the CEs, as shown in [Figure 1](#EN-US_TASK_0000001559771904__fig_dc_vrp_srv6_cfg_all_001101). For configuration details, see the configuration files.
...
4. Establish an EBGP peer relationship between each PE and its connected CE.
   ...
   ```
5. Create a locator on the PEs, configure intra-AS static End.X SIDs, and configure IS-IS to advertise SRv6 locator routes.
...
6. Configure BGP EPEv6 on ASBR1 and ASBR2, and allocate inter-AS static End.X SIDs.
   ...
   [~ASBR1] display bgp egress-engineering 
    ...
    SRv6 SID (PSP)                  : 2001:DB8:20::1:0:3             
    SRv6 SID (PSP,USP,USD)          : 2001:DB8:20::1:0:4            
    SRv6 SID (PSP,USP,USD,COC)      : --
    SRv6 SID (COC)                  : --
    SRv6 SID (PSP,COC)              : --
    SRv6 SID (PSP,USP,USD,COC,NEXT) : --
    SRv6 SID (PSP,USD,NEXT)         : --
...
10. Verify the configuration.
    ...
    ```
11. Create locator as2 on the PEs and configure IS-IS to advertise SRv6 locator routes.
    ...
    ```
    [~PE1] segment-routing ipv6
    [~PE1-segment-routing-ipv6] locator as2 ipv6-prefix 2001:DB8:100:1:: 64 compress block 48 compress-static 8 static 32
    [*PE1-segment-routing-ipv6-locator] opcode compress ::12 end psp-usp-usd
    [*PE1-segment-routing-ipv6-locator] quit
    [*PE1-segment-routing-ipv6] quit
    [*PE1] isis 1
    [*PE1-isis-1] segment-routing ipv6 locator as2 auto-sid-disable
    [*PE1-isis-1] commit
    [~PE1-isis-1] quit
    ```
    ...
    ```
    [~PE2] segment-routing ipv6
    [~PE2-segment-routing-ipv6] locator as2 ipv6-prefix 2001:DB8:100:4:: 64 compress block 48 compress-static 8 static 32
    [*PE2-segment-routing-ipv6-locator] opcode compress ::45 end psp-usp-usd
    [*PE2-segment-routing-ipv6-locator] quit
    [*PE2-segment-routing-ipv6] quit
    [*PE2] isis 10
    [*PE2-isis-10] segment-routing ipv6 locator as2 auto-sid-disable
    [*PE2-isis-10] commit
    [~PE2-isis-10] quit
    ```
12. Configure BGP EPEv6 on ASBR1 and ASBR2 to reference locator as2, and allocate inter-AS static End.X SIDs.
    ...
    ```
    [~ASBR1] segment-routing ipv6
    [~ASBR1-segment-routing-ipv6] locator as2 ipv6-prefix 2001:DB8:100:2:: 64 compress block 48 compress-static 8 static 32
    [*ASBR1-segment-routing-ipv6-locator] opcode compress ::22 end psp-usp-usd-coc
    [*ASBR1-segment-routing-ipv6-locator] quit
    [*ASBR1-segment-routing-ipv6] quit
    [*ASBR1] isis 1
    [*ASBR1-isis-1] segment-routing ipv6 locator as2 auto-sid-disable
    [*ASBR1-isis-1] commit
    [~ASBR1-isis-1] quit
    [~ASBR1] bgp 100 
    [~ASBR1-bgp] segment-routing ipv6 egress-engineering locator as2 
    [*ASBR1-bgp] peer 2001:DB8:3::2 egress-engineering srv6 static-sid psp-usp-usd-coc compress 2001:DB8:100:2:1::
    [*ASBR1-bgp] quit
    [*ASBR1] commit
    ```
    ...
    ```
    [~ASBR2] segment-routing ipv6
    [~ASBR2-segment-routing-ipv6] locator as2 ipv6-prefix 2001:DB8:100:3:: 64 compress block 48 compress-static 8 static 32
    [*ASBR2-segment-routing-ipv6-locator] opcode compress ::33 end psp-usp-usd-coc
    [*ASBR2-segment-routing-ipv6-locator] quit
    [*ASBR2-segment-routing-ipv6] quit
    [*ASBR2] isis 10
    [*ASBR2-isis-10] segment-routing ipv6 locator as2 auto-sid-disable
    [*ASBR2-isis-10] commit
    [~ASBR2-isis-10] quit
    [~ASBR2] bgp 200 
    [*ASBR2-bgp] segment-routing ipv6 egress-engineering locator as2 
    [*ASBR2-bgp] peer 2001:DB8:3::1 egress-engineering srv6 static-sid psp-usp-usd-coc compress 2001:DB8:100:3:1::
    [*ASBR2-bgp] quit
    [*ASBR2] commit
    ```
    
    After the configuration is complete, run the **display bgp egress-engineering** command to check BGP EPE information. The command output shows dual-locator SRv6 SID information. The following example uses the command output on ASBR1.
    ...
    [~ASBR1] display bgp egress-engineering 
     ...
     SRv6 SID (PSP,USD,NEXT)         : --
     Usable Locator                  : as2                 
     SRv6 SID                        : 2001:DB8:100:2:103::               
     SRv6 SID (PSP)                  : 2001:DB8:100:2:104::             
     SRv6 SID (PSP,USP,USD)          : 2001:DB8:100:2:105::            
     SRv6 SID (PSP,USP,USD,COC)      : 2001:DB8:100:2:1::
     SRv6 SID (COC)                  : --
     SRv6 SID (PSP,COC)              : --
     SRv6 SID (PSP,USP,USD,COC,NEXT) : --
     SRv6 SID (PSP,USD,NEXT)         : --
     ...
     Out Interface                   : GigabitEthernet0/2/0  
    ```
13. Configure an SRv6 TE Policy and add a segment list.
    ...
    [*PE1-segment-routing-ipv6-segment-list-list1] index 10 sid ipv6 2001:DB8:100:2:1:: compress block 48
    [*PE1-segment-routing-ipv6-segment-list-list1] index 15 sid ipv6 2001:DB8:100:4:45:: compress block 48 
    [*PE1-segment-routing-ipv6-segment-list-list1] commit
    [~PE1-segment-routing-ipv6-segment-list-list1] quit
    [~PE1-segment-routing-ipv6] srv6-te-policy locator as2 
    [*PE1-segment-routing-ipv6] srv6-te policy policy1 endpoint 2001:DB8:14::4 color 101
    [*PE1-segment-routing-ipv6-policy-policy1] binding-sid 2001:DB8:100:1::FFFF:FFFF
    [*PE1-segment-routing-ipv6-policy-policy1] candidate-path preference 200
    [*PE1-segment-routing-ipv6-policy-policy1-path] segment-list list2 
    [*PE1-segment-routing-ipv6-policy-policy1-path] commit
    [~PE1-segment-routing-ipv6-policy-policy1-path] quit
    [~PE1-segment-routing-ipv6-policy-policy1] quit
    [~PE1-segment-routing-ipv6] quit
    ...
    [*PE2-segment-routing-ipv6-segment-list-list1] index 10 sid ipv6 2001:DB8:100:3:1:: compress block 48         
    [*PE2-segment-routing-ipv6-segment-list-list1] index 15 sid ipv6 2001:DB8:100:1:12:: compress block 48 
    [*PE2-segment-routing-ipv6-segment-list-list1] commit
    [~PE2-segment-routing-ipv6-segment-list-list1] quit
    [~PE2-segment-routing-ipv6] srv6-te-policy locator as2 
    [*PE2-segment-routing-ipv6] srv6-te policy policy1 endpoint 2001:DB8:11::1 color 101
    [*PE2-segment-routing-ipv6-policy-policy1] binding-sid 2001:DB8:100:4::FFFF:FFFF 
    [*PE2-segment-routing-ipv6-policy-policy1] candidate-path preference 200
    [*PE2-segment-routing-ipv6-policy-policy1-path] segment-list list2 
    [*PE2-segment-routing-ipv6-policy-policy1-path] commit
    [~PE2-segment-routing-ipv6-policy-policy1-path] quit
    [~PE2-segment-routing-ipv6-policy-policy1] quit
    [~PE2-segment-routing-ipv6] quit
    ...
    TunnelId                : 1                              
    TunnelType              : SRv6-TE Policy                 DelayTimerRemain     : -            
    Policy State            : Up                             State Change Time    : 2023-02-17 11:16:30
    Admin State             : Up                             Traffic Statistics   : Disable   
    Backup Hot-Standby      : Disable                        BFD                  : Disable   
    Interface Index         : -                              Interface Name       : - 
    Interface State         : -                              Encapsulation Mode   : Insert
    Binding SID             : 2001:DB8:10::1000(Insert, Preferred), 2001:DB8:100:1::FFFF:FFFF(Insert)
    Candidate-path Count    : 2                                
    
     Candidate-path Preference : 200
     Path State             : Active                         Path Type            : Primary      
     Protocol-Origin        : Configuration(30)              Originator           : 0, 0.0.0.0   
     Discriminator          : 200                            Binding SID          : 2001:DB8:10::1000     
     GroupId                : 1                              Policy Name          : policy1
     Template ID            : 0                              Path Verification    : Disable
     DelayTimerRemain       : -                              Network Slice ID     : -
     Segment-List Count     : 1            
      Segment-List          : list2                            
       ...
       SID :        
             ...
             2001:DB8:100:2:1::                BL: 48   NL: -    FL: -    Compress Flavor: coc        Compress Length: 32
     ...
     Segment-List Count     : 1            
      Segment-List          : list1                            
       Segment-List ID      : 1                              XcIndex              : -            
       List State           : Down (Valid)                   DelayTimerRemain     : -    
       Verification State   : -                              SuppressTimeRemain   : -
       PMTU                 : 9600                           Active PMTU          : 9600    
       Weight               : 1                              BFD State            : -
       Loop Detection State : Up                             BFD Delay Remain     : - 
       ...
       SID :        
             ...
             2001:DB8:30::100
    ```
14. Switch the locator to as2 between the PEs to complete the evolution from non-compression to compression.
    ...
    [~PE1] bgp 100
    [~PE1-bgp] ipv4-family vpn-instance vpna
    [*PE1-bgp-vpna] segment-routing ipv6 locator as2 evpn
    [*PE1-bgp-vpna] commit
    [~PE1-bgp-vpna] quit
    [~PE1-bgp] quit
    ```
    ...
    [~PE2] bgp 200
    [~PE2-bgp] ipv4-family vpn-instance vpna
    [*PE2-bgp-vpna] segment-routing ipv6 locator as2 evpn
    [*PE2-bgp-vpna] commit
    [~PE2-bgp-vpna] quit
    [~PE2-bgp] quit
    ```
    
    After the configuration is complete, the VPN SID in the route received by the peer is switched to the SID of locator as2.
    
    ```
     [~PE2]display bgp evpn all routing-table prefix-route 0:10.1.1.0:24
     ...
     From: 2001:DB8:11::1 (1.1.1.1)
     Route Duration: 0d01h14m29s
     Relay IP Nexthop: FE80::3A02:FF:FE11:300
     Relay IP Out-Interface: Ethernet3/0/0
     Relay Tunnel Out-Interface:
     Original nexthop: 2001:DB8:11::1
     Qos information : 0x0
     Ext-Community: RT <111 : 1>, Color <0 : 101>
     Prefix-sid: 2001:DB8:100:1:0:1:0:21, Endpoint Behavior: DT4(19)
     AS-path 100, origin incomplete, MED 0, pref-val 0, valid, external, best, select, pre 255, IGP cost 10
...
* PE1 configuration file
  ...
  segment-routing ipv6
   ...
   locator as2 ipv6-prefix 2001:DB8:100:1:: 64 compress block 48 compress-static 8 static 32
    opcode compress ::12 end psp-usp-usd
   srv6-te-policy locator as1                             
   srv6-te-policy locator as2
   segment-list list1                                     
   ...
   segment-list list2                                     
    ...
    index 10 sid ipv6 2001:DB8:100:2:1:: compress block 48                           
   ...
   srv6-te policy policy1 endpoint 2001:DB8:14::4 color 101         
    ...
    binding-sid 2001:DB8:100:1::FFFF:FFFF
    candidate-path preference 200                         
     segment-list list2 
    candidate-path preference 100                         
     segment-list list1
  ...
  bgp 100         
   ...
   ipv4-family vpn-instance vpna
    ...
    segment-routing ipv6 locator as2 evpn //This command displays the post-switching configuration. If the command is run more than once, the latest configuration overrides the previous one. In a non-compression scenario, locator as1 is displayed.
...
* ASBR1 configuration file
  ...
  segment-routing ipv6 
   ...
   locator as2 ipv6-prefix 2001:DB8:100:2:: 64 compress block 48 compress-static 8 static 32
  ...
  bgp 100                                                 
   ...
   segment-routing ipv6 egress-engineering locator as2
   ...
   peer 2001:DB8:3::2 egress-engineering srv6 static-sid psp-usp-usd-coc compress 2001:DB8:100:2:1::
   # 
   ipv4-family unicast                                    
    undo synchronization  
   #         
...
* ASBR2 configuration file
  ...
  segment-routing ipv6  
   ...
   locator as2 ipv6-prefix 2001:DB8:100:3:: 64 compress block 48 compress-static 8 static 32
  ...
  bgp 200                                                 
   ...
   peer 2001:DB8:3::1 egress-engineering srv6 static-sid psp-usp-usd-coc compress 2001:DB8:100:3:1::
   #
   ipv4-family unicast                                    
    undo synchronization  
   #    
...
* PE2 configuration file
  ...
  segment-routing ipv6                                    
   ...
   locator as2 ipv6-prefix 2001:DB8:100:4:: 64 compress block 48 compress-static 8 static 32
   ...
   segment-list list2
    ...
    index 10 sid ipv6 2001:DB8:100:3:1:: compress block 48
   ...
   srv6-te policy policy1 endpoint 2001:DB8:11::1 color 101         
    ...
    binding-sid 2001:DB8:100:4::FFFF:FFFF
    candidate-path preference 200
     segment-list list2
    candidate-path preference 100                         
     segment-list list1 
  ...
  bgp 200                                                 
   ...
   ipv4-family vpn-instance vpna                          
    import-route direct   
    advertise l2vpn evpn                                
    segment-routing ipv6 locator as2 evpn //This command displays the post-switching configuration in a compression scenario. If the command is run more than once, the latest configuration overrides the previous one. In a non-compression scenario, locator as1 is displayed.
    segment-routing ipv6 traffic-engineer best-effort evpn     
