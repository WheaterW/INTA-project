Example for Configuring L3VPNv4 over Dynamic SRv6 TE Flow Group (IS-IS)
=======================================================================

This section provides an example for configuring a dynamic SRv6 TE flow group to carry L3VPNv4 services.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0000001106502586__fig184238288365):

* PE1, P1, P2, and PE2 are in the same AS. They are required to run IS-IS to achieve IPv6 network connectivity.
* PE1, P1, P2, and PE2 are Level-1 devices that belong to IS-IS process 1.

It is required that a bidirectional SRv6 TE flow group be deployed between PE1 and PE2 to carry L3VPNv4 services.

SRv6 TE flow groups are used to implement DSCP-based traffic steering. During route recursion, the ingress associates a service route with a specific SRv6 TE flow group based on the next-hop address of the route. The services carry DSCP information during forwarding. Based on the SRv6 mapping policy configuration, the services are first associated with a specific color and then a specific SRv6 TE Policy in the SRv6 TE flow group, thereby implementing DSCP-based traffic steering.
...
![](figure/en-us_image_0000001153622399.png)
...
6. Configure SRv6 SIDs and enable IS-IS SRv6 on PE1, P1, P2, and PE2. In addition, configure PE1 and PE2 to carry SIDs in VPN routes.
7. Deploy SRv6 TE Policies between PE1 and PE2.
8. Configure an SRv6 mapping policy on PE1 and PE2.
9. Configure a tunnel policy on PE1 and PE2 to preferentially use an SRv6 TE flow group for VPN traffic import.
...
1. Enable IPv6 forwarding and configure an IPv6 address for each interface.
   ...
   # Configure PE1. The configurations of P1, P2, and PE2 are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0000001106502586__example121226172748) in this section.
...
3. Configure a VPN instance on each PE, enable the IPv4 address family for the instance, and bind the interface that connects each PE to a CE to the VPN instance on that PE.
   ...
   # Configure an IP address for each interface on the CEs by referring to [Figure 1](#EN-US_TASK_0000001106502586__fig184238288365). For configuration details, see [Configuration Files](#EN-US_TASK_0000001106502586__example121226172748) in this section.
    ...
    Segment-List Count     : 1       
     Segment-List          : list2   
      ...
      SID :              
            ...
            2001:DB8:300::100  
   ```
8. Configure an SRv6 mapping policy.
   ...
   [*PE1-segment-routing-ipv6-mapping-policy-p1] match-type dscp
   [*PE1-segment-routing-ipv6-mapping-policy-p1-dscp] index 10 dscp ipv4 1 to 20 match srv6-te-policy color 10 
   [*PE1-segment-routing-ipv6-mapping-policy-p1-dscp] ipv4 default match srv6-te-policy color 20
   [*PE1-segment-routing-ipv6-mapping-policy-p1-dscp] quit
   ...
   [*PE2-segment-routing-ipv6-mapping-policy-p1] match-type dscp
   [*PE2-segment-routing-ipv6-mapping-policy-p1-dscp] index 10 dscp ipv4 1 to 20 match srv6-te-policy color 10 
   [*PE2-segment-routing-ipv6-mapping-policy-p1-dscp] ipv4 default match srv6-te-policy color 20
   [*PE2-segment-routing-ipv6-mapping-policy-p1-dscp] quit
...
9. Configure a tunnel policy to import VPN traffic.
   ...
   After the configuration is complete, run the **display srv6-te flow-group** command to check the status of the dynamically created SRv6 TE flow Group.
   ...
   Delay Timer Remain  : -                             UP/ALL Num          : 2/2                    
    ...
    Index                : 10                             AfType               : IPv4           
    DSCP                 : 1-20                                                 
    Match Tunnel         : SRv6-TE Policy                 State                : Up               
    Color                : 10                             Tunnel Id            : 1  
   ```
   
   After the configuration is complete, run the **display ip routing-table vpn-instance vpna** command to check the routing table of the specified VPN instance. The command output shows that VPN routes have successfully recursed to SRv6 TE Policies.
...
* PE1 configuration file
  ...
  segment-routing ipv6
   ...
   mapping-policy p1 color 101     
    match-type dscp 
     index 10 dscp ipv4 1 to 20 match srv6-te-policy color 10
