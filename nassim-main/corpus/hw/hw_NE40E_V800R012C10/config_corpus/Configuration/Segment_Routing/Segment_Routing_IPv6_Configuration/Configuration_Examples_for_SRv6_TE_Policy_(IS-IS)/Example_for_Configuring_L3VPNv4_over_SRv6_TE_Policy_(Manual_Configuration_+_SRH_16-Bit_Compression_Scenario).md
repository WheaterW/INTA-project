Example for Configuring L3VPNv4 over SRv6 TE Policy (Manual Configuration + SRH 16-Bit Compression Scenario)
============================================================================================================

This section provides an example for configuring an SRv6 TE Policy to carry L3VPNv4 services in a 16-bit compression scenario.
...
On the network shown in [Figure 1](#EN-US_TASK_0000001418254230__fig3691666259), PE1, P1, P2, and PE2 are in the same AS. They need to run IS-IS to achieve IPv6 network connectivity. A bidirectional SRv6 TE Policy needs to be deployed between PE1 and PE2 to carry L3VPNv4 services. In addition, to reduce the SRv6 header size, SRH compression is required.

**Figure 1** Networking diagram for configuring L3VPNv4 over SRv6 TE Policy in an SRH 16-bit compression scenario![](../../../../public_sys-resources/note_3.0-en-us.png) 

Interface1 and interface2 in this example represent GE 0/1/0 and GE 0/2/0, respectively.


  
![](figure/en-us_image_0000001468293377.png "Click to enlarge")
...
5. Establish a BGP VPNv4 peer relationship between the PEs.
   ...
   ```
   ```
   [*PE1-bgp-af-vpnv4] peer 2001:DB8:4::4 prefix-sid sid-type5 advertise-srv6-locator
   ```
   ```
   ...
   ```
   ```
   [*PE1-bgp-af-vpnv4] peer 2001:DB8:1::1 prefix-sid sid-type5 advertise-srv6-locator
   ```
   ```
...
6. Configure SRv6 SIDs and configure the PEs to advertise VPN routes carrying SIDs.
   ...
   ```
   ```
   [*PE1-bgp-vpna] segment-routing ipv6 locator PE1 compress
   ```
   ```
   [*PE1-bgp-vpna] commit
   ```
   ...
   ```
   ```
   [*PE2-bgp-vpna] segment-routing ipv6 locator PE2 compress
   ```
   ```
   [*PE2-bgp-vpna] commit
   ```
   ...
   StaticSIDEnd  : 2001:DB8:100:1:E000:0:FFFF:FFFF                                    
   GIB:LIB       : 14:2
    ...
    Segment-List Count     : 1
     Segment-List          : list1        
      ...
      SID :                               
            ...
            2001:DB8:100:4:E045::                  BL: 48  NL: 0   FL: 16  Compress Flavor: next     Compress Length: 16 
   ```
...
* PE1 configuration file
  ...
  bgp 100                                                    
   ...
   ipv4-family vpnv4
    ...
    peer 2001:DB8:4::4 prefix-sid sid-type5 advertise-srv6-locator
   #
   ipv4-family vpn-instance vpna                             
    import-route direct                                                                        
    segment-routing ipv6 locator PE1 compress                    
...
* PE2 configuration file
  ...
  bgp 100                                                    
   ...
   ipv4-family vpnv4
    ...
    peer 2001:DB8:1::1 prefix-sid sid-type5 advertise-srv6-locator
   #
   ipv4-family vpn-instance vpna                             
    import-route direct                                                                           
    segment-routing ipv6 locator PE2 compress                
    segment-routing ipv6 traffic-engineer best-effort   
