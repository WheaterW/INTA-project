...
* Configure IRB route advertisement.
  ...
  4. Run one of the following commands based on the overlay network type to configure IRB route advertisement:
     ...
     + For an IPv4 overlay network, run the [**peer**](cmdqueryname=peer+advertise+%28BGP-EVPN+address+family+view%29) { *ipv4-address* | *group-name* } **advertise** **irb** command.
     + For an IPv6 overlay network, run the [**peer**](cmdqueryname=peer+advertise+%28BGP-EVPN+address+family+view%29) { *ipv4-address* | *group-name* } **advertise** **irbv6** command.
