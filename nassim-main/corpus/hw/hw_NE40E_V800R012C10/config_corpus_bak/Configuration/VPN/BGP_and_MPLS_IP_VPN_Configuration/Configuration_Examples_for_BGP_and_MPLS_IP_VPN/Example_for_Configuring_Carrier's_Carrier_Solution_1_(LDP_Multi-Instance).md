...
The Level 1 carrier and Level 2 carrier are in the same AS. The Level 2 carrier provides the BGP/MPLS IP VPN service for its customers, as shown in [Figure 1](#EN-US_TASK_0172369526__fig_dc_vrp_mpls-l3vpn-v4_cfg_204201):
...
**Figure 1** Carrier's carrier solution 1![](../../../../public_sys-resources/note_3.0-en-us.png) 

Interfaces 1 and 2 in this example represent GE0/1/0 and GE0/2/0, respectively.


  
![](images/fig_dc_vrp_mpls-l3vpn-v4_cfg_204201.png)
...
1. The two types of routes are exchanged as follows:
   ...
   * Exchange of the external routes of the Level 2 carrier between the PE devices of the Level 2 carrier: Set up an MP-IBGP peer relationship between PEs (PE3 and PE4) of the Level 2 carrier.
2. Configure the carrier's carrier of the same AS and configure IGP and LDP between the Level 1 carrier PE and the Level 1 carrier CE.
...
1. Configure a BGP/MPLS IP VPN on the Level 1 carrier's backbone network.
   
   
   
   Configure IS-IS as IGP, enable LDP between PE1 and PE2, and establish an MP-IBGP peer relationship between them.
   ...
   Total Peer(s): 2
   ```
2. Configure the Level 2 carriers' networks.
   
   
   
   Configure OSPF as IGP, and enable LDP between PE3 and CE1 and between PE4 and CE2.
...
3. Configure Level 1 carrier CEs to access Level 1 carrier PEs.
   ...
   After the configurations are complete, PE1 and CE1 can establish an LDP peer relationship and an OSPF neighbor relationship.
   
   ![](../../../../public_sys-resources/note_3.0-en-us.png) 
   
   The configuration of the connection between PE2 and CE2 is similar to that of the connection between PE1 and CE1. For configuration details, see the configuration files.
4. Configure the Level 2 carrier's customers to access the Level 2 carrier PE.
   ...
   After the configurations are complete, the BGP peer relationship is set up successfully between CE3 and PE3, and the status is **Established**.
...
5. Establish an MP-IBGP peer relationship between Level 2 carrier PEs to exchange VPN routes of Level 2 carrier CEs.
   ...
   ```
   ```
   [~PE3] ip direct-routing-table local-host-route advertise enable
   ```
   ```
   [*PE3] commit
   ```
   ...
   The configuration of PE4 is similar to that of PE3. For configuration details, see the configuration files.
6. Verify the configuration.
   ...
   Routing Table: Public
     ...
     Destination/Mask  Proto  Pre  Cost       Flags  NextHop         Interface
          ...
          20.1.1.2/32  IBGP   255   0            RD  6.6.6.9         GigabitEthernet0/2/0
