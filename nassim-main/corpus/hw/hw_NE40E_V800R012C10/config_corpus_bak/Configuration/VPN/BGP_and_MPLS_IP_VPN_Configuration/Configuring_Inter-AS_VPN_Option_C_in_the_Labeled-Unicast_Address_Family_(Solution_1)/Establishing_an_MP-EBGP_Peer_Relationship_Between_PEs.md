...
* Configure an ASBR to send the loopback interface IP addresses of a PE used for peer relationship establishment to the ASBRs of other ASs and peer PEs. You can also configure a PE to advertise its loopback interface IP addresses used for peer relationship establishment to the peer PEs.
  ...
  3. Run [**ipv4-family labeled-unicast**](cmdqueryname=ipv4-family+labeled-unicast)
     
     
     
     The bgp labeled unicast address family view is displayed.
  4. Run [**network**](cmdqueryname=network) *ip-address* [ *mask* | *mask-length* ] [ **route-policy** *route-policy-name* ]
...
* (Optional) Disable an ASBR from advertising BGP supernet labeled routes.
  ...
  4. Run [**supernet label-route advertise**](cmdqueryname=supernet+label-route+advertise) **disable**
     ...
     After disabling the ASBR from advertising BGP supernet labeled routes, if you want the loopback addresses of PEs in the local AS to be advertised to PEs in another AS, run the [**network**](cmdqueryname=network) command for the ASBR to advertise BGP labeled routes carrying the loopback addresses of PEs in the local AS.
  5. Run [**commit**](cmdqueryname=commit)
     
     
     
     The configuration is committed.
* Import the labeled routes to the public network's unicast address family routing table.
  ...
  3. Run [**import-rib public labeled-unicast**](cmdqueryname=import-rib+public+labeled-unicast)
     ...
     The device is configured to import the labeled routes to the public network instance's unicast address family routing table.
...
* (Optional) Configure an RR.
  ...
  To improve scalability, specify an RR in each AS and establish MP-EBGP peer relationships between the RRs in ASs to save all VPNv4 routes on the RRs. Then configure PEs in each AS as RR clients to exchange VPNv4 routing information with the local RR. Perform the following steps on each device that needs to serve as an RR:
  ...
  4. Run [**peer**](cmdqueryname=peer) *ipv4-address1* [**connect-interface**](cmdqueryname=connect-interface) **loopback** *interface-number*
     
     
     
     The source interface for sending BGP packets to the peer RR is specified.
  5. Run [**peer**](cmdqueryname=peer) *ipv4-address1* **ebgp-max-hop** [ *hop-count* ]
     
     
     
     The maximum number of hops allowed for establishing an EBGP peer relationship is configured.
  6. Run [**ipv4-family**](cmdqueryname=ipv4-family) **vpnv4** [ **unicast** ]
  ...
  9. Run [**peer**](cmdqueryname=peer) *ipv4-address2* **as-number** { *as-number-plain* | *as-number-dot* }
     
     
     
     The device is configured as an RR, and PEs in the local AS are specified as its IBGP clients.
  10. Run [**peer**](cmdqueryname=peer) *ipv4-address2* [**connect-interface**](cmdqueryname=connect-interface) **loopback** *interface-number*
      
      
      
      The source interface for sending BGP packets to PEs in the local AS is specified.
  11. Run [**peer**](cmdqueryname=peer) *ipv4-address2* [**reflect-client**](cmdqueryname=reflect-client)
