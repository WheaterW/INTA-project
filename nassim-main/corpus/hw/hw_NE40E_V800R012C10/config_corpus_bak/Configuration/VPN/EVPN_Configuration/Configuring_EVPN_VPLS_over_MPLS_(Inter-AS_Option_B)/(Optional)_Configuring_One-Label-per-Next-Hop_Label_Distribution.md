...
In an inter-AS EVPN Option B scenario, after one-label-per-next-hop label distribution is configured on an ASBR, the ASBR assigns only one label to EVPN routes that share the same next hop and outgoing label. Compared with one-label-per-route label distribution, one-label-per-next-hop label distribution significantly saves label resources on the ASBR.
...
4. Run [**apply-label per-nexthop**](cmdqueryname=apply-label+per-nexthop)
   ...
   After this function is enabled or disabled on an ASBR, the labels assigned by the ASBR to routes change. As a result, temporary packet loss may occur.
