Example for Configuring the IPv6 NFVI Distributed Gateway Function (BGP EVPN over E2E SR Tunnels)
=================================================================================================
...
[Figure 1](#EN-US_TASK_0172370680__fig_01) shows NFVI distributed gateway networking (BGP EVPN over E2E SR tunnels). DC GWs, which are the border gateways of the DCN, exchange Internet routes with external devices through PEs. L2GW/L3GW1 and L2GW/L3GW2 connect to VNFs. VNF1 and VNF2 that function as virtualized NEs are deployed to implement the vUGW and vMSE functions, respectively. VNF1 and VNF2 each connect to L2GW/L3GW1 and L2GW/L3GW2 through IPUs. BGP EVPN peer relationships are established between PEs and L2GWs/L3GWs and between DC GWs and L2GWs/L3GWs, and DC GWs are deployed as EVPN RRs. E2E SR tunnels are established between PEs and L2GWs/L3GWs to carry IPv6 service traffic.

**Figure 1** Configuring the IPv6 NFVI distributed gateway function (BGP EVPN over E2E SR tunnels)![](../../../../public_sys-resources/note_3.0-en-us.png) 
...
13. Configure load balancing on DC GWs and L2GWs/L3GWs.
...
1. Configure IP addresses for all interfaces, including loopback interfaces, on PEs, DC GWs, and L2GWs/L3GWs.
   ...
   For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370680__dc_vrp_evpn_cfg_014901).
...
5. Configure EVPN instances on L2GWs/L3GWs and bind BDs to the EVPN instances.
   ...
   The configuration of L2GW/L3GW2 is similar to that of L2GW/L3GW1. For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370680__dc_vrp_evpn_cfg_014901).
6. Configure L3VPN instances on PEs, DC GWs, and L2GWs/L3GWs and bind VBDIF interfaces to these L3VPN instances on L2GWs/L3GWs.
   ...
   ```
   ```
   [*PE1-vpn-instance-vpn1-af-ipv6] vpn-target 11:1 evpn
   ```
   ```
   ...
   The configuration of PE2 is similar to that of PE1. For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370680__dc_vrp_evpn_cfg_014901).
   ...
   The configuration of DC-GW2 is similar to that of DC-GW1. For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370680__dc_vrp_evpn_cfg_014901).
...
7. Configure a tunnel policy on PEs, DC GWs, and L2GWs/L3GWs.
   ...
   The configuration of L2GW/L3GW2 is similar to that of L2GW/L3GW1. For detailed configurations, see [Configuration Files](#EN-US_TASK_0172370680__dc_vrp_evpn_cfg_014901).
8. Configure BGP EVPN on PEs, DC GWs, and L2GWs/L3GWs and configure L2GWs/L3GWs as RRs.
   ...
   ```
   [~PE1] ipv6 route-static vpn-instance vpn1 :: 0 NULL0 tag 2000
   ```
   ```
   [*PE1] bgp 200
   ```
...
11. Configure and apply a route-policy for L3VPN instances on PEs so that PEs can delete the GWIP attribute carried in the VNF routes received from EVPN peers.
    ...
    ```
    [~PE1] route-policy delGWIPv6 permit node 10
    ```
    ```
    [*PE1-route-policy] apply ipv6 gateway-ip none
    ```
    ```
    ...
    ```
    ```
    [*PE1-vpn-instance-vpn1-af-ipv6] import route-policy delGWIPv6 evpn
    ```
    ```
    [*PE1-vpn-instance-vpn1-af-ipv6] quit
    ```
...
12. On DC GWs, configure the loopback addresses used to establish BGP VPN peer relationships with VNFs. Configure and apply a route-policy for L3VPN instances so that DC GWs can only advertise VPN loopback routes and UE routes (in this example, the destination IP addresses of UE routes destined for the VNF are 10.10.10.10) through BGP EVPN and the GWIP attribute is added to UE routes and deleted from the routes received from EVPN peers.
    ...
    ```
    ```
    [*DCGW1] route-policy dpv6 permit node 5
    ```
    ```
    [*DCGW1-route-policy] if-match ipv6 address prefix-list uIPv6
    ```
    ...
    ```
    ```
    [*DCGW1-route-policy] if-match ipv6 address prefix-list lpv6
    ```
    ```
    [*DCGW1-route-policy] quit
    ```
    ```
    [*DCGW1] route-policy delGWIPv6 permit node 10
    ```
    ```
    ...
    ```
    ```
    [*DCGW1-vpn-instance-vpn1-af-ipv6] import route-policy delGWIPv6 evpn
    ```
    ```
    [*DCGW1-vpn-instance-vpn1-af-ipv6] quit
    ```
...
* PE1 configuration file
  ...
  ip vpn-instance vpn1
   ipv6-family
    ...
    import route-policy delGWIPv6 evpn
  ...
  #               
  route-policy delGWIPv6 permit node 10
...
* DC-GW1 configuration file
  ...
  route-policy delGWIPv6 permit node 10
   apply ipv6 gateway-ip none
  #
  route-policy dpv6 permit node 5
