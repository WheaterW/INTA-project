Example for Configuring EVPN L3VPN HoVPN over MPLS
==================================================

This section provides an example for configuring EVPN L3VPN HoVPN over MPLS to deliver network connectivity.

#### Networking Requirements

An IP bearer network generally uses L2VPN and L3VPN (HVPN) to carry Layer 2 and Layer 3 services, respectively. The protocols are complex. EVPN, in contrast, can carry both Layer 2 and Layer 3 services. To simplify service bearer protocols, many IP bearer networks will evolve to EVPN. Among them, L3VPN HVPN that carries Layer 3 services needs to evolve to EVPN L3VPN HVPN. On the network shown in [Figure 1](#EN-US_TASK_0172370661__fig126551753125815), an access layer network is deployed between the UPE and SPE, and an aggregation layer network is deployed between the SPE and NPE. Independent IGP protocols need to be deployed on the access layer network and aggregation layer network for connectivity. EVPN L3VPN HoVPN needs to be deployed to implement E2E connectivity. On an EVPN L3VPN HoVPN, a UPE does not have specific routes to the NPE and can only send service data to the SPE over default routes. As a result, route isolation is implemented. An EVPN L3VPN HoVPN can use devices with relatively poor route management capabilities as UPEs, reducing network deployment costs.

**Figure 1** Configuring EVPN L3VPN HoVPN over MPLS![](../../../../public_sys-resources/note_3.0-en-us.png) 

Interfaces 1 and 2 in this example represent GE0/1/0 and GE0/2/0, respectively.


  
![](figure/en-us_image_0000001179235002.png)  
...
1. Configure IP addresses, including loopback interface addresses, on the UPE, SPE, and NPE.
   ...
   For details about how to configure interface IP addresses and masks, see [Configuration Files](#EN-US_TASK_0172370661__dc_vrp_evpn_cfg_008501) in this section.
2. Deploy an IGP between the UPE and SPE and between the SPE and NPE. In this example, deploy OSPF between the UPE and SPE and IS-IS between the SPE and NPE.
   ...
   For configuration details, see [Configuration Files](#EN-US_TASK_0172370661__dc_vrp_evpn_cfg_008501) in this section.
...
9. Verify the configuration.
   ...
   Destination/Mask    Proto   Pre  Cost        Flags NextHop         Interface
      ...
      192.168.20.0/24  Direct  0    0             RD  192.168.20.1    GigabitEthernet0/2/0
      192.168.20.1/32  Direct  0    0             D   127.0.0.1       GigabitEthernet0/2/0
    192.168.20.255/32  Direct  0    0             D   127.0.0.1       GigabitEthernet0/2/0
   255.255.255.255/32  Direct  0    0             D   127.0.0.1       InLoopBack0
   ```
