Example for Configuring EVPN Carrier's Carrier (LDP Label Distribution, Labeled Route Mode)
===========================================================================================
...
On the network shown in [Figure 1](#EN-US_TASK_0000001168513242__fig_dc_vrp_mpls-l3vpn-v4_cfg_204401), the Level 1 and Level 2 carriers are in different ASs, and the Level 2 carriers provide BGP/MPLS IP VPN services for their customers.

After learning labeled public network BGP routes from Level 1 carrier devices, Level 2 carrier devices import these routes into the IGP. LDP can then distribute labels for these routes and trigger the establishment of a complete LDP LSP between Level 2 carrier PEs. In this way, inter-AS carrier's carrier is implemented.
...
![](figure/en-us_image_0000001213911671.png)  
...
5. Configure LDP to distribute labels for labeled public network BGP routes on Level 1 carrier CEs.
   ...
   After the configuration is complete, run the **display ip routing-table** *dest-ip-address* **verbose** command on CE1. The command output shows that the routes from CE1 to PE4 are labeled public network BGP routes: **Routing Table** is **Public**, **Protocol** is **BGP**, and **Label** has a non-zero value.
