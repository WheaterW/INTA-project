Example for Configuring Heterogenous BGP VPWS Between Ethernet and PPP Links
============================================================================

If one CE connects to a PE over an Ethernet link and another CE connects to another PE over a PPP link, you can configure heterogeneous BGP VPWS for the two CEs to communicate.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0172369948__fig_dc_vrp_vpws_cfg_606201), CE1 connects to PE1 over an Ethernet link, and CE2 connects to PE2 over a POS link. A heterogeneous remote BGP VPWS connection needs to be established between PE1 and PE2 for CE1 and CE2 to communicate.

**Figure 1** Configuring BGP VPWS heterogeneous interworking![](../../../../public_sys-resources/note_3.0-en-us.png) 

In this example, interfaces 1 through 3 represent GE0/1/0, GE0/2/0, and POS0/1/0, respectively.


  
![](images/fig_dc_vrp_vpws_cfg_606201.png)  
...
1. Configure IP addresses.
   ...
   ```
   ```
   [~CE2-Pos0/1/0] ip address 10.1.1.2 24
   ```
   ```
   [*CE2-Pos0/1/0] quit
   ```
   ...
   ```
   ```
   [*P-GigabitEthernet0/2/0] ip address 192.168.10.2 24
   ```
   ```
...
2. Configure an IGP. In this example, OSPF is used.
   ...
   [*PE2] commit
   ```
3. Enable MPLS and establish an LSP.
...
5. Configure a remote BGP VPWS connection.
   ...
   ```
   ```
   [~PE1-GigabitEthernet0/1/0] local-ce ip 10.1.1.1
   ```
   ```
   [*PE1-GigabitEthernet0/1/0] quit
   ```
   ...
   ```
   ```
   [*PE1] mpls l2vpn vpn1 encapsulation ip-interworking 
   ```
   ```
   ...
   ```
   ```
   [*PE2] mpls l2vpn vpn1 encapsulation ip-interworking
   ```
   ```
   ...
   ```
   ```
   [*PE2-mpls-l2vpn-vpn1-ce-ce2] connection ce-offset 1 interface pos 0/1/0
   ```
   ```
...
6. Verify the configuration.
   ...
   After the configurations are complete, run the [**display mpls l2vpn connection**](cmdqueryname=display+mpls+l2vpn+connection) command on each PE. The command output shows that a VPWS connection has been established and is in the up state. The following example uses the command output on PE1.
   ...
   Ping CE2 from CE1. The ping operation is successful.
...
* PE2 configuration file
  ...
  mpls l2vpn vpn1 encapsulation ip-interworking
   ...
   ce ce2 id 2 range 10 default-offset 0
    connection ce-offset 1 interface Pos0/1/0
  #               
  bgp 100         
