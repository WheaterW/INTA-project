Example for Configuring Inter-AS L3VPNv4 over SRv6 TE Policy (IS-IS)
====================================================================
...
On the network shown in [Figure 1](#EN-US_TASK_0224822522__fig_dc_vrp_srv6_cfg_all_001101):
...
![](figure/en-us_image_0224823233.png "Click to enlarge")
...
2. Enable IS-IS, configure an IS-IS level, and specify a network entity title (NET) on PE1, PE2, ASBR1, and ASBR2.
...
1. Enable IPv6 forwarding and configure an IPv6 address for each interface.
   ...
   # Configure PE1. The configurations of PE2, ASBR1, and ASBR2 are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0224822522__section_dc_vrp_srv6_cfg_all_001105) in this section.
...
3. Configure a VPN instance on each PE, enable the IPv4 address family for the instance, and bind the interface that connects each PE to a CE to the VPN instance on that PE.
   ...
   # Configure an IP address for each interface on the CEs, as shown in [Figure 1](#EN-US_TASK_0224822522__fig_dc_vrp_srv6_cfg_all_001101). For configuration details, see [Configuration Files](#EN-US_TASK_0224822522__section_dc_vrp_srv6_cfg_all_001105) in this section.
...
6. Configure BGP EPEv6 on ASBR1 and ASBR2, and allocate inter-AS static End.X SIDs.
   ...
   After the configuration is complete, run the **display bgp egress-engineering** command on each ASBR to check BGP EPE information. The command output shows SRv6 SID information. The following example uses the command output on ASBR1.
...
10. Verify the configuration.
    ...
    Run the **ping** command on a CE. The command output shows that CEs belonging to the same VPN can ping each other. For example:
