Example for Configuring Inter-AS L3VPNv4 over OSPFv3 SRv6 TE Policy (Manual Configuration)
==========================================================================================

This section provides an example for configuring an inter-AS SRv6 TE Policy to carry L3VPNv4 services.

#### Networking Requirements

On the network shown in [Figure 1](#EN-US_TASK_0000001171437036__fig_dc_vrp_srv6_cfg_all_001101):

* PE1 and ASBR1 belong to AS 100, and PE2 and ASBR2 belong to AS 200. Intra-AS IPv6 connectivity needs to be achieved for AS 100 and AS 200 through OSPFv3.
* PE1 and ASBR1 belong to OSPFv3 process 1, and PE2 and ASBR2 belong to OSPFv3 process 10. PE1, ASBR1, PE2, and ASBR2 are all Area 0 devices.

It is required that a bidirectional inter-AS SRv6 TE Policy be deployed between PE1 and PE2 to carry L3VPNv4 services.

**Figure 1** Inter-AS L3VPNv4 over SRv6 TE Policy networking![](../../../../public_sys-resources/note_3.0-en-us.png) 

Interfaces 1 and 2 in this example represent GE 0/1/0 and GE 0/2/0, respectively.


  
![](figure/en-us_image_0000001171118558.png "Click to enlarge")
...
2. Enable OSPFv3 and configure area IDs on PE1, PE2, ASBR1, and ASBR2.
3. Configure VPN instances on PE1 and PE2.
4. Establish an EBGP peer relationship between each PE and its connected CE.
5. Configure the PEs to advertise SRv6 locator routes through OSPFv3 and allocate intra-AS static End.X SIDs to the PEs.
6. Configure BGP EPEv6 on ASBR1 and ASBR2, and allocate inter-AS static End.X SIDs.
7. Establish an MP-EBGP peer relationship between the PEs, and configure the PEs to exchange BGP VPNv4 routes carrying SIDs through this peer relationship.
...
* OSPF process IDs and area IDs of the PEs and ASBRs
...
1. Enable IPv6 forwarding and configure an IPv6 address for each interface.
   ...
   # Configure PE1. The configurations of PE2, ASBR1, and ASBR2 are similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0000001171437036__section_dc_vrp_srv6_cfg_all_001105) in this section.
...
2. Configure OSPFv3.
   ...
   ```
   [~ASBR1] ospfv3 1 
   [*ASBR1-ospfv3-1] router-id 2.2.2.2
   [*ASBR1-ospfv3-1] area 0
   [*ASBR1-ospfv3-1] quit
   [*ASBR1] interface gigabitethernet 0/1/0
   [*ASBR1-GigabitEthernet0/1/0] ospfv3 1 area 0.0.0.0
   [*ASBR1-GigabitEthernet0/1/0] quit
   [*ASBR1] interface loopback1
   [*ASBR1-LoopBack1] ospfv3 1 area 0.0.0.0
   ...
   ```
   [~ASBR2] ospfv3 10 
   [*ASBR2-ospfv3-10] router-id 3.3.3.3
   [*ASBR2-ospfv3-10] area 0
   [*ASBR2-ospfv3-10] quit
   [*ASBR2] interface gigabitethernet 0/1/0
   [*ASBR2-GigabitEthernet0/1/0] ospfv3 10 area 0.0.0.0
   [*ASBR2-GigabitEthernet0/1/0] quit
   [*ASBR2] interface loopback1
   [*ASBR2-LoopBack1] ospfv3 10 area 0.0.0.0
   ...
   ```
   [~PE2] ospfv3 10
   [*PE2-ospfv3-10] router-id 4.4.4.4
   [*PE2-ospfv3-10] area 0
   [*PE2-ospfv3-10] quit
   [*PE2] interface gigabitethernet 0/1/0
   [*PE2-GigabitEthernet0/1/0] ospfv3 10 area 0.0.0.0
   [*PE2-GigabitEthernet0/1/0] quit
   [*PE2] interface loopback1
   [*PE2-LoopBack1] ospfv3 10 area 0.0.0.0
...
3. Configure a VPN instance on each PE, enable the IPv4 address family for the instance, and bind the interface that connects each PE to a CE to the VPN instance on that PE.
   ...
   # Configure an IP address for each interface on the CEs, as shown in [Figure 1](#EN-US_TASK_0000001171437036__fig_dc_vrp_srv6_cfg_all_001101). For configuration details, see [Configuration Files](#EN-US_TASK_0000001171437036__section_dc_vrp_srv6_cfg_all_001105) in this section.
...
5. Configure the PEs to advertise SRv6 locator routes through OSPFv3 and allocate intra-AS static End.X SIDs to the PEs.
   ...
   [*PE1] ospfv3 1
   [*PE1-ospfv3-1] segment-routing ipv6 locator as1 auto-sid-disable
   [*PE1-ospfv3-1] commit
   [~PE1-ospfv3-1] quit
   ```
   ...
   [*PE2] ospfv3 10 
   [*PE2-ospfv3-10] segment-routing ipv6 locator as1 auto-sid-disable
   [*PE2-ospfv3-10] commit
   [~PE2-ospfv3-10] quit
   ```
...
6. Configure BGP EPEv6 on ASBR1 and ASBR2, and allocate inter-AS static End.X SIDs.
   ...
   [*ASBR1] ospfv3 1
   [*ASBR1-ospfv3-1] segment-routing ipv6 locator as1 auto-sid-disable
   [*ASBR1-ospfv3-1] import-route bgp
   [*ASBR1-ospfv3-1] commit
   [~ASBR1-ospfv3-1] quit
   [~ASBR1] bgp 100 
   ...
   [*ASBR2] ospfv3 10
   [*ASBR2-ospfv3-10] segment-routing ipv6 locator as1 auto-sid-disable
   [*ASBR2-ospfv3-10] import-route bgp
   [*ASBR2-ospfv3-10] commit
   [~ASBR2-ospfv3-10] quit
   [~ASBR2] bgp 200 
   ...
   [~ASBR1] display bgp egress-engineering                  
    ...
    SRv6 SID (PSP)           : 2001:DB8:20::1:0:1E                  
...
7. Establish an MP-EBGP peer relationship between the PEs, and configure the PEs to exchange BGP VPNv4 routes carrying SIDs through this peer relationship.
   ...
   [~PE1] display bgp vpnv4 all peer                        
    ...
    Total number of peers : 2                 Peers in established state : 2       
     ...
     Peer            V          AS  MsgRcvd  MsgSent  OutQ  Up/Down       State  PrefRcv                                               
     2001:DB8:14::4  4         200     1032     1037     0 14:58:13 Established        2                                               
...
9. Configure a tunnel policy to import VPN traffic.
   ...
   [*PE1] bgp 100
   [*PE1-bgp] ipv4-family vpnv4
   [*PE1-bgp-af-vpnv4] peer 2001:DB8:14::4 route-policy p1 import 
   [*PE1-bgp-af-vpnv4] quit
   [*PE1-bgp] quit
   ...
   Destination: 22.22.22.22/32                             
      ...
      Preference: 255                      Cost: 0         
         NextHop: 2001:DB8:4::4       Neighbour: 2001:DB8:14::4      
...
10. Verify the configuration.
    ...
    [~CE1] ping -a 11.11.11.11 22.22.22.22
      ...
      --- 22.22.22.22 ping statistics ---                   
        ...
        5 packet(s) received                                
        0.00% packet loss                                   
        round-trip min/avg/max = 30/233/1030 ms 
    ```
