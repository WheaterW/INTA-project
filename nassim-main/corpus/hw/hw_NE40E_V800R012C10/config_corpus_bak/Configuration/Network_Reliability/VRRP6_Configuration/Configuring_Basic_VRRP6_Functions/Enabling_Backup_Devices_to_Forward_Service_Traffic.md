...
On the mobile bearer network shown in [Figure 1](#EN-US_TASK_0000001192220119__fig_dc_vrp_vrrp_cfg_012401), the NodeB attached to the cell site gateway (CSG) is connected to PE1 and PE2 over the primary and secondary pseudo wires (PWs) and to PE3 and PE4 over the active and standby links. A Virtual Router Redundancy Protocol (VRRP6) group is deployed on both PE3 and PE4. If PE1 fails, service traffic switches from the active link to the standby link immediately, but a master/backup VRRP6 switchover is performed between PE3 and PE4 after a delay. Therefore, service traffic is lost before the master/backup VRRP6 switchover is performed.
...
**Figure 1** Enabling PE4 to forward service traffic
  
![](images/fig_dc_vrp_vrrp6_cfg_012401.png)  
...
3. Run [**vrrp6 vrid**](cmdqueryname=vrrp6+vrid) *virtual-router-id* **backup-forward**
