Example for Configuring MVPNv6 over BIERv6 Network Slicing on an L3VPNv6 over SRv6 Network
==========================================================================================

This section provides an example for configuring MVPNv6 over BIERv6 and BIERv6 network slicing on a unicast L3VPNv6 over SRv6 network.

#### Networking Requirements

In [Figure 1](#EN-US_TASK_0000001386557416__fig35284409229), L3VPNv6 over SRv6 has been deployed. It is required that MVPNv6 over BIERv6 be deployed on the existing network. Multicast service traffic in VPNA needs to be isolated from other multicast service traffic in BIERv6 tunnels. In addition, the multicast stream with the multicast source address 2001:DB8:4::100 and multicast group address FF1E::1 in VPNA needs to be isolated from other multicast streams in VPNA, and independent SLA assurance is implemented. In this case, the network slicing function can be deployed in BIERv6 tunnels to carry different multicast service traffic through different slice planes.

**Figure 1** MVPNv6 over BIERv6 network slicing networking![](../../../../public_sys-resources/note_3.0-en-us.png) 

In this example, interface1, interface2, and interface3 represent GE 0/1/0, GE 0/1/1, and GE 0/1/2, respectively.


  
![](figure/en-us_image_0000001436836697.png "Click to enlarge")
#### Configuration Roadmap

The configuration roadmap is as follows:

1. (Optional) Configure L3VPNv6 over SRv6 and ensure that the unicast VPN runs properly. If the unicast network has been configured, skip this step.
...
* Addresses of key interfaces on each device, as shown in [Figure 1](#EN-US_TASK_0000001386557416__fig35284409229)
...
* Network slice ID (20) for VPNA, and network slice ID (30) for the (2001:DB8:4::100, FF1E::1) multicast stream in VPNA
...
1. (Optional) Configure L3VPNv6 over SRv6.
   ...
   # Configure a VPN instance on the PEs, enable the IPv6 address family for the instance, and bind the interfaces that connect the PEs to a CE to the VPN instance. The configuration of PE1 is used as an example. The configurations of PE2 and PE3 are similar to the configuration of PE1.
   ...
   [*PE1-GigabitEthernet0/1/1] ipv6 address 2001:db8:4::1 96
   ...
   * Configure PE1.
     ...
     [*PE1-bgp-6-vpna] peer 2001:db8:4::2 as-number 65410
   ...
   [*PE1-bgp-af-vpnv6] peer 2001:db8:20::1 enable
   [*PE1-bgp-af-vpnv6] peer 2001:db8:30::1 enable
   ...
   [*PE1] bgp 100
   [*PE1-bgp] ipv6-family vpnv6
   [*PE1-bgp-af-vpnv6] peer 2001:db8:20::1 prefix-sid
   [*PE1-bgp-af-vpnv6] peer 2001:db8:30::1 prefix-sid
   [*PE1-bgp-af-vpnv6] quit
   [*PE1-bgp] ipv6-family vpn-instance VPNA
   [*PE1-bgp-6-vpna] segment-routing ipv6 best-effort
   [*PE1-bgp-6-vpna] segment-routing ipv6 locator PE1
   [*PE1-bgp-6-vpna] commit
   [~PE1-bgp-6-vpna] quit
   [~PE1-bgp] quit
   [~PE1] isis 1
...
6. Enable PIM on PEs.
   ...
   [*PE1-Loopback2] ipv6 address 2001:db8:50::1 32
   [*PE1-Loopback2] pim ipv6 sm 
   [*PE1-Loopback2] quit
   [*PE1] pim-ipv6 vpn-instance VPNA
   [*PE1-pim6-VPNA] static-rp 2001:db8:50::1
   [*PE1-pim6-VPNA] quit
   [*PE1] commit
   ```
   ...
   [*PE2-pim6-VPNA] static-rp 2001:db8:50::1
   [*PE2-pim6-VPNA] quit
   [*PE2] commit
   ```
   ...
   [*PE3] pim-ipv6 vpn-instance VPNA
   [*PE3-pim6-VPNA] static-rp 2001:db8:50::1
   [*PE3-pim6-VPNA] quit
   [*PE3] commit
   ```
...
9. Specify a network slice ID for a VPN instance and another network slice ID for a multicast stream in the VPN instance on PE1.
   ...
   [~PE1-vpn-instance-VPNA] ipv6-family
   [~PE1-vpn-instance-VPNA-af-ipv6] mvpn
   [~PE1-vpn-instance-VPNA-af-ipv6-mvpn] bier ipv6 network-slice 20
   [*PE1-vpn-instance-VPNA-af-ipv6-mvpn] bier ipv6 group FF1E::1 source 2001:DB8:4::100 network-slice 30
   [*PE1] commit
   ```
10. Verify the configuration.
    
    Run the **display mvpn ipv6 all-instance ingress bier ipv6** command on the ingress PE1 of the BIERv6 tunnel to check BIERv6 stream information. The command output shows that the network slice ID corresponding to the multicast stream (2001:DB8:4::100, FF1E::1) is 30, and the network slice ID corresponding to other multicast streams is 20.
    ```
    <PE1> display mvpn ipv6 all-instance ingress bier ipv6
