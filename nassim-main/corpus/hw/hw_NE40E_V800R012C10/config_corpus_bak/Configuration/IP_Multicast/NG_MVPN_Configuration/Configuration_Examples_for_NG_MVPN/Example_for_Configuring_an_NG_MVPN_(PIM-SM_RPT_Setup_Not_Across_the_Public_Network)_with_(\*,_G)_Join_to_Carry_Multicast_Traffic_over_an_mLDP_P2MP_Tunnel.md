Example for Configuring an NG MVPN (PIM-SM RPT Setup Not Across the Public Network) with (\*, G) Join to Carry Multicast Traffic over an mLDP P2MP Tunnel
=========================================================================================================================================================

This section provides an example for configuring an NG MVPN (PIM-SM RPT setup not across the public network) with (\*, G) Join to carry multicast traffic over an mLDP P2MP tunnel.

#### Networking Requirements

On the single-AS NG MVPN shown in [Figure 1](#EN-US_TASK_0000001270313493__fig_dc_vrp_cfg_ngmvpn_001901), MPLS LDP tunnels have been deployed to carry BGP/MPLS IP VPN services. In this NG MVPN configuration example, users join a multicast group through PIM-SM (\*, G) Join messages, and PIM-SM RPTs are set up not across the public network.
...
4. Configure PE1 to use mLDP to establish an I-PMSI tunnel so that an mLDP P2MP tunnel is established. Configure PIM-SM RPTs to be set up not across the public network.
...
6. Configure a candidate-rendezvous point (C-RP) and candidate-bootstrap router (C-BSR) on each PE.
   ...
   * If PIM-SM RPT setup is not across the public network and is triggered by (\*, G) Join messages, you can use either static or dynamic RPs. Either a PE or CE can function as an RP. If a CE functions as a private network RP, establish an MSDP peer relationship between the CE and the corresponding PE. For details about the MSDP peer configuration, see MSDP Configuration. In addition, you need to configure import and export of MSDP SA messages and BGP Source Active AD routes. For configuration details, see [Configuring a P2MP LSP to Carry Multicast Traffic](dc_vrp_cfg_ngmvpn_0007.html).
...
1. Configure BGP/MPLS IP VPN.
   1. Assign an IP address to each interface of devices on the VPN backbone network and VPN sites.
      ...
      Assign an IP address to each interface according to [Figure 1](#EN-US_TASK_0000001270313493__fig_dc_vrp_cfg_ngmvpn_001901). For configuration details, see [Configuration Files](#EN-US_TASK_0000001270313493__example_dc_vrp_cfg_ngmvpn_001901) in this section.
   2. Configure an IGP for interworking on the BGP/MPLS IP VPN backbone network.
      
      
      
      OSPF is used in this example. For configuration details, see [Configuration Files](#EN-US_TASK_0000001270313493__example_dc_vrp_cfg_ngmvpn_001901) in this section.
   3. Configure basic MPLS functions and enable MPLS LDP to establish LDP LSPs on the MPLS backbone network.
      ...
      The configuration of PE2 is similar to the configuration of PE1. For configuration details, see [Configuration Files](#EN-US_TASK_0000001270313493__example_dc_vrp_cfg_ngmvpn_001901) in this section.
   ...
   7. # Configure OSPF on each CE.
      ...
      The configuration of CE2 is similar to the configuration of CE1. For configuration details, see [Configuration Files](#EN-US_TASK_0000001270313493__example_dc_vrp_cfg_ngmvpn_001901) in this section.
      
      After the configurations are complete, run the [**display ip routing-table**](cmdqueryname=display+ip+routing-table) command on CE2. The command outputs show that CE2 has routes to CE1. Run the [**ping**](cmdqueryname=ping) command on CE2 to ping CE1. The command outputs show the ping operations are successful.
...
3. Establish a BGP MVPN peer relationship between the PEs.
   ...
   After completing the configurations, run the [**display bgp mvpn all peer**](cmdqueryname=display+bgp+mvpn+all+peer) command on the PEs. The command output shows that PE1 has established a BGP MVPN peer relationship with PE2. The following example uses the command output on PE1:
   ...
   ```
    ...
    Total number of peers : 1                 Peers in established state : 1
     ...
     2.2.2.2         4         100       36       29     0 00:14:21 Established    2
   ```
4. Configure devices to use mLDP to establish an I-PMSI tunnel, and configure PIM-SM RPTs to be set up not across the public network.
...
8. Verify the configuration.
   ...
   ```
    ...
    (10.1.3.100, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: pseudo
                Protocol: BGP, UpTime: 00:02:48, Expires: -
   ```
   ```
   ...
   ```
    ...
    (*, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: Gigabitethernet0/1/1
                Protocol: pim-sm, UpTime: 00:15:56, Expires: 00:02:34
    ...
    (10.1.3.100, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: Gigabitethernet0/1/1
                Protocol: pim-sm, UpTime: 00:05:03, Expires: 00:02:56
   ```
   
   Run the [**display pim routing-table**](cmdqueryname=display+pim+routing-table) command on CE1 and CE2 to check information about their PIM routing tables.
   ...
   ```
    ...
    (10.1.3.100, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: Gigabitethernet0/1/1
                Protocol: pim-sm, UpTime: 00:03:47, Expires: 00:02:33
   ```
   ```
   ...
   ```
    ...
    (*, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: Gigabitethernet0/1/1
                Protocol: static, UpTime: 04:22:50, Expires: -
    ...
    (10.1.3.100, 225.0.0.1)
        ...
        Total number of downstreams: 1
           1: Gigabitethernet0/1/1
                Protocol: pim-sm, UpTime: 00:11:16, Expires: -
   ```
   ...
   The command output shows that an mLDP P2MP tunnel has been established, with PE1 as the root node and PE2 as a leaf node.
