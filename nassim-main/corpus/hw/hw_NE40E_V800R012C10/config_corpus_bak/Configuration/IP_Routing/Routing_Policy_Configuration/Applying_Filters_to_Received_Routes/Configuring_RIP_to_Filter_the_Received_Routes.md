...
3. Set the conditions to filter the received routes.
   ...
   * Based on the basic ACL:
     ...
     4. Run [**rip**](cmdqueryname=rip) [ *process-id* ]
        ...
        A RIP process is created, and the RIP view is displayed.
     5. Run [**filter-policy**](cmdqueryname=filter-policy) { *acl-number* | **acl-name** *acl-name* } **import** [ *interface-type* *interface-number* ]
   ...
   * Based on the IP prefix list:
     ...
     Run [**filter-policy**](cmdqueryname=filter-policy) **ip-prefix** *ip-prefix-name* **import** [ *interface-type* *interface-number* ]
