...
The **timeout** command sets the timeout period of packets in an NQA test instance of DNS test type.
...
It is recommended to set the timeout period based on the RTT value. The timeout period configured using the **timeout** command must be greater than the RTT value.The timeout period set using the **timeout** command must be less than or equal to the interval for sending NQA test packets configured using the **interval** command. Otherwise, packets time out.
