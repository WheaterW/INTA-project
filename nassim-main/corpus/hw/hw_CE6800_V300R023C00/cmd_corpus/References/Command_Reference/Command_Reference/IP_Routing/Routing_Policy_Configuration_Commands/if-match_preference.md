...
The **if-match preference** command configures a route priority-based matching rule.

The **undo if-match preference** command deletes a route priority-based matching rule.



By default, no route priority-based matching rule is configured.
...
| --- | --- | --- |
| *preference* | Specifies a route priority. | The value is an integer ranging from 0 to 255. |
...
You can use the **if-match preference** command to configure a node to filter routes based on the route preference. After such a filtering rule is configured, you can apply the apply clauses to change the attributes of the routes that match the filtering rule.
...
# Configure a matching rule to match the routes with the priority being 50.
```
<HUAWEI> system-view
[~HUAWEI] route-policy policy permit node 10
[*HUAWEI-route-policy] if-match preference 50
