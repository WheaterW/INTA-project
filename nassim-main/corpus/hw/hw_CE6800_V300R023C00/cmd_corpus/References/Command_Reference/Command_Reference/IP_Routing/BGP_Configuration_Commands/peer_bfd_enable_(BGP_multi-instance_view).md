...
**peer** { *ipv4-address* | *ipv6-address* } **bfd** **enable** [ **single-hop-prefer** ] [ **compatible** ]

**peer** { *ipv4-address* | *ipv6-address* } **bfd** **enable** **per-link** **one-arm-echo**

**peer** { *ipv4-address* | *ipv6-address* } **bfd** **enable** **one-arm-echo**

**undo peer** { *ipv4-address* | *ipv6-address* } **bfd** **enable**
...
Before enabling BFD on a BGP peer, enable BFD in the system view. If no BFD detection parameter is specified, the BFD session uses the default values.If the command is run more than once, the latest configuration overrides the previous one.Note:The **peer bfd block** command and the **peer bfd enable** command are mutually exclusive. After the **peer bfd block** command is run, the BFD session is automatically deleted.
