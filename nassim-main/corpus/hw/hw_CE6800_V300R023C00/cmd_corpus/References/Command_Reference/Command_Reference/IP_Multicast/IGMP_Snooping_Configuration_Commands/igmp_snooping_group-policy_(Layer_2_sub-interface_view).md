igmp snooping group-policy (Layer 2 sub-interface view)
=======================================================
...
If no IGMP version is specified, the device applies the multicast group address limit to the received IGMP messages of all versions (V1, V2, and V3). In addition, the configured ACL must be a basic ACL.If you run this command in the VLAN view and the dot1q sub-interface has been bound to the VLAN, the configuration fails.
...
# Allow hosts connected to 100GE 1/0/1.1 to join only multicast group 225.1.1.123.
```
<HUAWEI> system-view
