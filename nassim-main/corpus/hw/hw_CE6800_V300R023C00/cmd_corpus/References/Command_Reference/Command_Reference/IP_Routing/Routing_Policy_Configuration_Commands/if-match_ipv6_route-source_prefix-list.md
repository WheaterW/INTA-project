...
When you run the **if-match ipv6 route-source prefix-list** command to match the address of the source from which IPv6 routes are received, an IPv6 prefix list must also be configured for the matching rule to take effect. For example:

* If if-match ipv6 route-source prefix-list aa is configured but ip ipv6-prefix aa is not configured, all routes are permitted.
* If if-match ipv6 route-source prefix-list aa and ip ipv6-prefix aa permit 2001:db8:1::1 128 are configured, the routes with the next hop being 2001:db8:1::1 are permitted.
