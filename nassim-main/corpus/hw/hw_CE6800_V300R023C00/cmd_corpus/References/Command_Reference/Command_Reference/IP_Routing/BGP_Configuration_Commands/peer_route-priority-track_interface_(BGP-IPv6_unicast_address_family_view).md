...
In a BGP over M-LAG networking scenario, you can run this command on an M-LAG device. If an M-LAG member interface (connected to a specified peer) on the M-LAG device fails, this configuration allows BGP to detect the Eth-Trunk interface fault and change the priority of the routes received from the peer to the lowest (by setting the MED to the maximum value and the Local\_Pref to the minimum value). When multiple M-LAG member interfaces on the device are specified, BGP changes the priority of the received routes to the lowest only if all these member interfaces go faulty.
