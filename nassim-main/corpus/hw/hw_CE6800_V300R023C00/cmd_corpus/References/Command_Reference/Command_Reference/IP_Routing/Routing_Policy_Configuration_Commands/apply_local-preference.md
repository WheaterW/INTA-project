...
The Local-Pref attribute is a proprietary attribute of BGP, used to determine the optimal route when traffic leaves an AS. When a BGP device obtains multiple routes with the same destination address but different next hops through IBGP peers, the route with the largest Local\_Pref value is selected. The **apply local-preference** command sets only the local preference for BGP routes.
...
If a BGP route matches a route-policy, the local preference value specified in the route-policy is set for the BGP route. Setting the local preference value affects the BGP route selection result, which may cause a routing loop.
...
The Local\_Pref attribute is used for route selection only within an AS and is not advertised outside the AS. Therefore, the **apply local-preference** command does not take effect in an export policy when the policy is used to filter routes to be advertised to an EBGP peer.After the **apply local-preference** command is run in a route-policy or its configuration is changed, route updates are triggered.
