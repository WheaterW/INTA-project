...
| --- | --- | --- |
| **rate-limit** *rate-limit* | Specifies a rate limit for sending ND NA messages on an interface. | The value is an integer, in messages/second. The value ranges from 1 to 10240. |
...
When a device is attacked, it receives a large number of messages within a short period of time. As a result, the CPU resources of the device are greatly consumed, affecting the processing of other services. To solve this problem, you can configure the rate limit for sending ND NA messages on a specified interface. The device then counts the number of ND NA messages sent on the interface in a specified period. If the number of ND NA messages sent in a specified period exceeds the configured rate limit, the device delays scheduling or ignores excess ND NA messages.
...
[*HUAWEI-Eth-Trunk1] ipv6 nd na send rate-limit 550
