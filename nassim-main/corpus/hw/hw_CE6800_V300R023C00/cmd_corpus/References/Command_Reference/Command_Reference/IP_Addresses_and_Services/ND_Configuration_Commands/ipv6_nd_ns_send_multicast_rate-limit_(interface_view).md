ipv6 nd ns send multicast rate-limit (interface view)
=====================================================
...
**ipv6 nd ns send multicast rate-limit** *rate-limit*

**undo ipv6 nd ns send multicast rate-limit** *rate-limit*
...
| --- | --- | --- |
| **rate-limit** *rate-limit* | Specifies a rate limit for sending ND NS multicast messages on an interface. | The value is an integer, in messages/second. The value ranges from 1 to 1000. |
| **ns** | Specifies a rate limit for sending NS multicast messages on an interface. | - |
...
When a device is attacked, it receives a large number of messages within a short period of time. As a result, the CPU resources of the device are greatly consumed, affecting the processing of other services. To solve this problem, you can set a rate limit for sending ND NS multicast messages on a specified interface. The device then counts the number of ND NS multicast messages sent on the interface in a specified period. If the number of ND NS multicast messages exceeds the configured rate limit, the device delays scheduling or ignores excess ND NS multicast messages.
...
[*HUAWEI-Eth-Trunk1] ipv6 nd ns send multicast rate-limit 550
