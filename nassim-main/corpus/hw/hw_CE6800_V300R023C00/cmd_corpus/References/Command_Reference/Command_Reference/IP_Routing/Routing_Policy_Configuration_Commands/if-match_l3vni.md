...
The **if-match l3vni** command configures a matching rule based on a Layer 3 VNI list.

The **undo if-match l3vni** command cancels the configuration.



By default, no matching rule based on a Layer 3 VNI list is configured.
...
**undo if-match l3vni** [ *l3vni-list-name* ]
...
| --- | --- | --- |
| *l3vni-list-name* | Specifies the name of a Layer 3 VNI list. If l3vni-list-name is not specified, a route is matched if only it carries Layer 3 VNI information. | The value is a string of 1 to 63 case-sensitive characters. It cannot contain spaces. |
...
To configure a matching rule based on a Layer 3 VNI list, run the **if-match l3vni** command.
...
# Configure a matching rule based on a Layer 3 VNI list.
```
<HUAWEI> system-view
[~HUAWEI] filter-list vni abc
[*HUAWEI-vni-list-abc] quit
[*HUAWEI] route-policy policy permit node 10
[*HUAWEI-route-policy] if-match l3vni abc
