...
| **source-ip** *ipv6-address* | Specifies a source IP address. | The address is a 32-digit hexadecimal number, in the format of X:X:X:X:X:X:X:X. |
| **maximum** *max-value* | Specifies a rate limit for sending ND messages to the CPU based on a specified source IP address. | The value is an integer ranging from 0 to 5000, in pps. |
