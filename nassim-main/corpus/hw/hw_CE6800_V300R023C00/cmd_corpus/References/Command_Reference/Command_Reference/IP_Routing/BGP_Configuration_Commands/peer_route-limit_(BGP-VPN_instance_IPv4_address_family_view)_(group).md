...
| *percentage* | Specifies the percentage threshold of the number of received routes to the maximum number of routes that can be received.   * If the actual percentage is greater than percentage and less than 100%, the device sends a threshold alarm BGP\_1.3.6.1.4.1.2011.5.25.177.1.3.1 ROUTETHRESHOLDEXCEED and can receive routes. * When the percentage reaches 100%, the device sends a threshold-crossing alarm BGP\_1.3.6.1.4.1.2011.5.25.177.1.3.6 ROUTEEXCEED and stops receiving routes. | The value is an integer ranging from 1 to 100. The default value is 75. |
