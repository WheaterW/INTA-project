...
An RD has been configured for the VPN instance IPv4/IPv6 address family using the **route-distinguisher** command.If the associated route-policy does not exist, run the **route-policy** command to configure the route-policy.

**Configuration Impact**

The current VPN instance IPv4/IPv6 address family can be associated with only one import route-policy. If this command is run more than once, the latest configuration overrides the previous one.The **import route-policy** command does not affect the import route-policy configured using the **import route-policy** command.
