...
| *acl-name* | Specifies the name of a named basic ACL. | The value is a string of 1 to 32 case-sensitive characters without spaces. The value starts with a letter or digit but cannot contain only digits. |
...
When you run the **if-match ipv6 address acl** command to match the destination address of an IPv6 route, the matching condition must be used together with an ACL. For example:

* If if-match ipv6 address acl 2000 is configured but acl ipv6 name 2000 is not configured, all routes are permitted.
* If if-match ipv6 address acl 2000 is configured, acl ipv6 name 2000 is configured, and the ACL rule is rule 1 permit source 2001:db8:1::1 128, the routes with the next hop 2001:db8:1::1 are permitted.
