...
| *mask-length* | Specifies the mask length of an IP address. If no mask length is specified, the IP address is considered as a classful address. | It is an integer ranging from 0 to 128. |
| **route-policy** *route-policy-name* | Specifies the name of a route-policy that is used for route advertisement. | The value is a string of 1 to 200 case-sensitive characters. It cannot contain spaces. When double quotation marks are used around the string, spaces are allowed in the string. |
