...
**peer** { *ipv4-address* | *ipv6-address* } **public-as-only** **force** [ **replace** ] [ **include-peer-as** ]

**undo peer** { *ipv4-address* | *ipv6-address* } **public-as-only** **force** [ **replace** ] [ **include-peer-as** ]
...
| **replace** | Replaces private AS numbers in an AS\_Path list with a local AS number:  If both force and replace are specified, private AS numbers in an AS\_Path list, except the AS number of a specified peer or peer group, are replaced with the local AS number. | - |
...
# Enable a device to delete all private AS numbers from the AS\_Path attribute except the private AS number of a specified peer when the device sends BGP Update messages to the peer.
```
<HUAWEI> system-view
[~HUAWEI] bgp 100
