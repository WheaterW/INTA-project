...
| --- | --- | --- |
| *preferredVal* | Specifies the preferred value of BGP routes. In route selection, the BGP route with the largest preferred value is preferred. | The value is an integer in the range from 0 to 65535. |
...
If a BGP route matches a route-policy, the preferred value specified in the route-policy is set for the BGP route. Setting the preferred value affects the BGP route selection result, which may cause a routing loop.
