...
The **apply tag** command sets a tag for a route.

The **undo apply tag** command cancels the setting.
...
| --- | --- | --- |
| *tag* | Specifies the tag of routes. Routes can be tagged as required. You can set the same tag for routes of the same type. Then, routes can be flexibly controlled and managed through tags in the route-policy. | The value is an integer ranging from 0 to 4294967295. |
...
To set the same tag for the routes that match the filtering conditions specified in the if-match clause, run the **apply tag** command.
...
After a route matches a route-policy, the tag of the route is changed.
...
The **apply tag** command sets the tag for only IGP routes.
