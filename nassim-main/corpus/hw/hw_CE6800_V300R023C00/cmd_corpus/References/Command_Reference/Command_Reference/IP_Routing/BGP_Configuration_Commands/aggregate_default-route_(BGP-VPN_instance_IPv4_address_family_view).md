...
The **aggregate default-route** command enables a device to summarize the routes that match a specified IPv4 prefix list into a summary default route.

The **undo aggregate default-route** command restores the default configuration.



By default, BGP does not summarize routes into a summary default route according to an IPv4 prefix list.
...
| --- | --- | --- |
| **attribute-policy** *attribute-policy-name* | Specifies the name of an attribute route-policy. | The value is a string of 1 to 200 characters. |
...
# Enable a device to summarize the routes that match an IPv4 prefix list into a summary default route.
```
<HUAWEI> system-view
...
# Apply a route-policy to summary default route generation.
```
<HUAWEI> system-view
