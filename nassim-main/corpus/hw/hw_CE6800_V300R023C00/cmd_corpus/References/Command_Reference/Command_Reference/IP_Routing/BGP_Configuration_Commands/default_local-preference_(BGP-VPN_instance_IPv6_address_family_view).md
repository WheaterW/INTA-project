...
| --- | --- | --- |
| *preference* | Specifies the local preference for BGP routes. The larger the value, the higher the priority. | The value is an integer ranging from 0 to 4294967295. The default value is 100. |
...
1. The **default local-preference** command sets the default local preference for all the routes (except reflected routes) advertised by the local device to all IBGP peers. The local preference is valid only between IBGP peers and is not advertised to other ASs.
2. In load balancing scenarios other than EIBGP load balancing and load balancing between BGP routes and routes of other protocols, if routes do not carry the Local\_Pref attribute, the value 0 is used for load balancing, instead of the default Local\_Pref attribute configured on the local device.
