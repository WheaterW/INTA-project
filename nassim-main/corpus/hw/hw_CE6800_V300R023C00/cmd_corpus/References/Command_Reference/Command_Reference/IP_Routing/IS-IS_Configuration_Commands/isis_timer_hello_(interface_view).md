...
| --- | --- | --- |
| *hello-interval* | Specifies an interval at which Hello packets are sent. | The value is an integer ranging from 1 to 255, in seconds. |
| **level-1** | Indicates an interval at which Level-1 Hello packets are sent. | - |
| **level-2** | Indicates an interval at which Level-2 Hello packets are sent.  If neither Level-1 nor Level-2 is specified, the default level is Level-1 and Level-2.  level-1 and level-2 can be configured only on IS-IS broadcast interfaces. | - |
...
# Set the interval at which Level-2 Hello packets are sent to 20 seconds on Gigabit 100GE1/0/1.
```
<HUAWEI> system-view
[~HUAWEI] isis 1
[*HUAWEI-isis-1] quit
