...
The **if-match tag** command sets a filtering rule that is based on the route tag.

The **undo if-match tag** command cancels the configuration.



By default, no filtering rule based on the route tag is set.
...
| --- | --- | --- |
| *tag* | Indicates the tag value. | The value is an integer ranging from 0 to 4294967295. |
...
To filter routes based on the tags, run the **if-match tag** command.
...
BGP does not have the tag attribute. Therefore, the **if-match tag** command cannot be used to match BGP routes.
