...
**ipv6 import-route** { **direct** | **static** | { **ospfv3** | **ripng** | **isis** } [ *process-id* ] | **bgp** [ **permit-ibgp** ] } [ **cost** *cost* | **tag** *tag* | { **route-policy** *route-policy-name* } | [ **level-1** | **level-2** | **level-1-2** ] ] \*

**ipv6 import-route** { { **ospfv3** | **ripng** | **isis** } [ *process-id* ] | **bgp** [ **permit-ibgp** ] | **direct** } **inherit-cost** [ { **level-1** | **level-2** | **level-1-2** } | **tag** *tag* | **route-policy** *route-policy-name* ] \*
...
| **permit-ibgp** | Imports IBGP routes in a public network instance.  If you do not specify permit-ibgp, the device imports EBGP routes only. | - |
...
1. Importing routes of other protocols may cause routing loops. Therefore, exercise caution when using this command. You are advised to use a routing policy when importing routes.
2. After a policy for importing routes is configured using the route-policy parameter in the command, the IS-IS process can import only the network segment routes that meet the policy. This prevents the device from passively importing unnecessary routes. If no policy is specified for importing routes, a large number of unexpected routes may be imported. As a result, the number of routes exceeds the upper limit or the memory is overloaded.
