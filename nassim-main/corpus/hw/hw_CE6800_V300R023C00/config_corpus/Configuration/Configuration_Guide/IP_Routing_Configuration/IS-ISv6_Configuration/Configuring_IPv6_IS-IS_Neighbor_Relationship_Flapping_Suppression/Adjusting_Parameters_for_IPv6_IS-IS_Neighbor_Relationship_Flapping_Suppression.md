Adjusting Parameters for IPv6 IS-IS Neighbor Relationship Flapping Suppression
==============================================================================
...
Before adjusting parameters for IPv6 IS-IS neighbor relationship flapping suppression, you have completed the following task:

* [Configure basic IPv6 IS-IS functions](vrp_isis_ipv6_cfg_0011.html).

#### Context

If an IPv6 IS-IS interface alternates between up and down, IPv6 IS-IS neighbor relationship flapping occurs on the interface. During the flapping, IPv6 IS-IS frequently sends IIHs to reestablish the neighbor relationship, synchronizes LSDBs, and recalculates routes. In this process, a large number of packets are exchanged, impacting neighbor relationship stability, IPv6 IS-IS services, and IPv6 IS-IS-dependent services. IPv6 IS-IS neighbor relationship flapping suppression can address this problem by either delaying IS-IS neighbor relationship reestablishment, or setting the link cost to the maximum value to prevent service traffic from passing through flapping links.
...
4. Set the flapping suppression mode to Hold-down.
   ...
   IPv6 IS-IS neighbor relationship flapping suppression works in either Hold-down or Hold-max-cost mode.
   ...
   * Hold-max-cost mode: If the traffic forwarding path frequently changes, interfaces use the maximum cost (16777214 in wide mode and 63 in narrow mode) of the flapping link during the suppression period to prevent traffic from passing through the flapping link.
...
5. Configure detection parameters for IPv6 IS-IS neighbor relationship flapping suppression.
   ...
   Each IPv6 IS-IS interface maintains a flapping counter. If the interval between two successive neighbor state changes from Up to Initial or Down is less than or equal to the detecting-interval, a valid flapping\_event is recorded, and the flapping\_count increases by 1. When the flapping\_count reaches or exceeds the *threshold*, flapping suppression takes effect. If the interval between two successive periods when the neighbor state is Initial or Down is greater than or equal to *resume-interval*, neighbor relationship flapping suppression exits, and the flapping\_count is set to 0.
   
   ![](public_sys-resources/note_3.0-en-us.png) 
   
   The value of *resume-interval* must be greater than that of *detecting-interval*.
   
   You can set parameters for the detection of IPv6 IS-IS neighbor relationship flapping suppression on a specified interface according to the actual network situation. By default, the detection interval of IPv6 IS-IS neighbor relationship flapping suppression is 60s, the suppression threshold is 10, and the interval for exiting flapping suppression is 120s. Using the default values is recommended.
