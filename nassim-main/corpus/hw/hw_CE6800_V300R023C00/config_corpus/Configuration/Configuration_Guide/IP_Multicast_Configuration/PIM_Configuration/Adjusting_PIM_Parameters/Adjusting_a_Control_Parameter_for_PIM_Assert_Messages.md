Adjusting a Control Parameter for PIM Assert Messages
=====================================================
...
Before adjusting the control parameters for PIM Assert messages, enable IP multicast routing in the public network instance.


#### Context

When multiple multicast data forwarders are available on a shared network segment, only one forwarder is elected from them through the Assert mechanism. The devices that fail in the election do not forward multicast data through their downstream interfaces within the Assert holdtime until the holdtime expires.
...
* Perform interface-specific configuration.
  ...
  4. Set an Assert holdtime for the interface.
     ...
     ```
     [pim holdtime assert](cmdqueryname=pim+holdtime+assert) interval
     ```
