...
Either static or dynamic RPs can be used on a network. You are advised to configure RPs on loopback interfaces. You need to configure the same RP address on the devices to be deployed with anycast RP.
...
3. Configure an anycast RP and enter its view.
   ...
   The anycast RP address must be the same as the address of the RP elected on the network.
